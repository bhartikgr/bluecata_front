...
declare -A CLIENT_MARKERS=(
  ...
  ["warrants_options_v2349"]="v23.4.9 markers (warrants 3-way + share-price auto-calc)"
)
...
banner "Summary"
if [ "$FAILED" -eq 0 ]; then
  echo "${GREEN}${BOLD}ALL MARKERS PRESENT.${RESET} Capavate v23.4.9 is fully deployed on $PUBLIC_URL."
  exit 0
else
  echo "${RED}${BOLD}$FAILED MARKER(S) FAILED.${RESET} v23.4.9 is NOT fully deployed on $PUBLIC_URL."
  echo ""
  echo "Next steps:"
