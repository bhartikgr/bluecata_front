...
const deleteWarrantData = (warrantData) => {
  return new Promise((resolve, reject) => {
    const { roundrecord_id, company_id } = warrantData;

    // First get warrant IDs
    const getWarrantIdsSql = `SELECT id FROM warrants WHERE company_id = ? AND roundrecord_id = ?`;

    db.query(
      getWarrantIdsSql,
      [company_id, roundrecord_id],
      (err, warrants) => {
        if (err) {
        ...
        `;
          ...
          db.query(
            ...
            (err2) => {
              if (err2) {
                // Continue with warrant deletion even if this fails
              }

              // Delete from warrants
              const deleteWarrantSql = `DELETE FROM warrants WHERE company_id = ? AND roundrecord_id = ?`;
              ...
              db.query(
                ...
                (err3, result) => {
                  if (err3) {
                    console.error("Error deleting warrant:", err3);
                    return reject(err3);
                  }
                  resolve({
                    success: true,
      ...
      if (hasWarrants_preferred === false) {
        ...
        db.query(
          ...
          (err, warrants) => {
            ...
            const warrantIds = warrants[0]?.id;
            // Update warrants
            const updateSql = `
              UPDATE warrants 
              SET 
...
// Helper function to update/create warrant data (internal use only)
const updateWarrantDataInternal = (warrantData) => {
  return new Promise((resolve, reject) => {
    const {
      roundrecord_id,
      company_id,
      ...
      warrantType,
      warrant_fixed_shares,
      warrant_coverage_percentage_main,
    } = warrantData;
    let formattedExpirationDate = null;
    if (expiration_date) {
      formattedExpirationDate = formatDateForMySQL(expiration_date);
    }
    const checkSql =
      "SELECT * FROM warrants WHERE roundrecord_id = ? AND company_id = ?";
        ...
        `;
          ...
          db.query(insertSql, insertValues, (insertErr, insertResult) => {
            ...
            resolve({
              success: true,
              operation: "insert",
              warrantId: insertResult.insertId,
            });
          });
        } else {
...
// Function to convert date from 'DD/MonthName/YYYY' to 'YYYY-MM-DD'
const formatDateForMySQL = (dateString) => {
  ...
  // Handle format: '02/April/2026' or '02-April-2026'
  ...
  // Handle format: '02/04/2026' (DD/MM/YYYY)
  ...
  uploadFields(req, res, async (err) => {
        ...
        async (err, results) => {
          ...
          db.query(sql, values, async (err) => {
            ...
            if (hasWarrants_preferred === "true") {
              const warrantdata = {
                warrant_coverage_percentage:
                  parseFloat(warrant_coverage_percentage) || 0,
                warrant_exercise_type:
                  warrant_exercise_type || "next_round_adjusted",
                warrant_adjustment_percent:
                  parseFloat(warrant_adjustment_percent) || 0,
                warrant_adjustment_direction:
                  warrant_adjustment_direction || "decrease",
                warrant_status: "pending",
                warrantType: warrantType,
                warrant_fixed_shares: warrant_fixed_shares,
                expiration_date: expirationDate_preferred || null,
                roundrecord_id: id,
                company_id: company_id,
                share_class_type: shareClassType,
                instrument_type: instrumentType,
                round_name: nameOfRound,
                warrant_coverage_percentage_main:
                  parseFloat(warrant_coverage_percentage_main) || 0,
              };
              try {
                await updateWarrantDataInternal(warrantdata);
              } catch (warrantErr) {
                // Don't fail the main operation, just log the error
              }
            }
            if (hasWarrants_preferred === "false") {
              const warrantUpdateDataa = {
                warrant_coverage_percentage:
                  parseFloat(warrant_coverage_percentage) || 0,
                warrant_exercise_type:
                  warrant_exercise_type || "next_round_adjusted",
              ...
              try {
                await deleteWarrantData(warrantUpdateDataa);
              } catch (warrantErr) {
                // Don't fail the main operation, just log the error
              }
            }
    ...
    // ============================================================
      ...
      db.query(sql, values, async (err, result) => {
        ...
        var currentRound = req.body;
        if (hasWarrants_preferred === "true") {
          const warrantInsertData = {
            roundrecord_id: newId,
            company_id: company_id,
            investor_id: 0,
            warrant_coverage_percentage:
              parseFloat(warrant_coverage_percentage) || 0,
          ...
          try {
            await createWarrantDataInternal(warrantInsertData);
          } catch (warrantErr) {
            // Continue execution even if warrant creation fails
          }
        }
    ...
    for (const round of allRounds) {
      try {
        ...
        const total_shares_before = actualBeforeData
          ? parseFloat(actualBeforeData.total_shares_before)
          : parseFloat(roundData.calculated_total_before) || 0;
        ...
        const round0_shares = round0Data
          ? parseFloat(round0Data.total_founder_shares)
          : 100000;
  ...
  db.query(
    ...
    async (err, results) => {
      ...
      const previous_investment_shares =
        parseFloat(results[0]?.previous_investment_shares) || 0;
      const previous_option_shares =
        parseFloat(results[0]?.previous_option_shares) || 0;
      const total_converted_shares =
        parseFloat(results[0]?.total_converted) || 0;
      ...
      let total_shares_before =
        parseFloat(results[0]?.total_shares_before_calc) || 0;
...
// Function to get total shares sum with cap_table_type = 'post'
// Add this function at the top of your controller file
const getTotalSharesSumService = async (company_id, round_id) => {
  return new Promise((resolve, reject) => {
    const query = `
      SELECT 
        COALESCE((
          SELECT SUM(CAST(shares AS DECIMAL(20,6))) 
          FROM round_founders 
          WHERE company_id = ? 
            AND round_id = ? 
        ...
        COALESCE((
          ...
          WHERE company_id = ? 
            AND round_id = ? 
            AND cap_table_type = 'post'
            AND investor_type NOT IN ('warrant', 'warrant not exercised')
        ), 0) +
        COALESCE((
          SELECT SUM(CAST(shares AS DECIMAL(20,6))) 
          FROM round_option_pools 
          WHERE company_id = ? 
            AND round_id = ? 
            AND cap_table_type = 'post'
        ), 0) AS total_shares
    ...
    db.query(
      query,
      [company_id, round_id, company_id, round_id, company_id, round_id],
      (err, results) => {
        if (err) {
          console.error("Error getting total shares sum:", err);
          reject(err);
        } else {
          resolve({
            success: true,
            data: {
              total_shares: parseFloat(results[0]?.total_shares) || 0,
            },
          });
        }
      },
...
async function handleCommonStockCalculation(params, updateFlag = false) {
  ...
  let total_shares_before = params.total_shares_before || 0;
  const isUpdate = updateFlag || isUpdateFromParams;

  // Parse values as FLOAT (not parseInt) to preserve decimals
  ...
  let round0Shares = parseFloat(round0_shares) || 0;
  ...
  try {
    ...
    if (round0Data && round0Data.founder_data) {
      ...
      totalFounderShares = parseFloat(round0Data.total_founder_shares) || 0;
      ...
      if (founderData.founders && Array.isArray(founderData.founders)) {
        founderList = founderData.founders.map((founder, index) => ({
          ...
          investment: parseFloat(founder.investment) || 0,
          share_price: parseFloat(founder.share_price) || 0,
  ...
  if (round0Shares === 0 && totalFounderShares > 0) {
    round0Shares = totalFounderShares;
  }
  ...
  const hasInvestmentRoundBefore =
  ...
  const isPreviousRoundRound0 =
    previousRounds.length > 0 && latestPreviousRound?.round_type === "Round 0";
  ...
  if (previousRounds.length > 0 && latestPreviousRound) {
    ...
    const befopredata = await getTotalSharesSumService(company_id, round.id);
    // ✅ Use a different variable name (not reassigning const)
    const previousRoundTotalShares = befopredata.data.total_shares;

    // If you need to use it later, assign to a let variable
    total_shares_before = befopredata.data.total_shares;
  }
  ...
  let previousWarrantsList = [];
  let previousWarrantsTotalShares = 0;
  let currentRoundWarrantsList = [];
  let currentRoundWarrantsTotalShares = 0;
  let totalWarrantShares = 0;
  let warrantCalculationResults = [];

  if (previousRounds.length > 0 && latestPreviousRound) {
    const round = latestPreviousRound;
    existingOptionPoolShares =
      parseFloat(round.total_option_pool) ||
      parseFloat(round.option_pool_shares) ||
      0;
    totalPreMoneyShares = parseFloat(total_shares_before) || round0Shares;
    ...
    const roundInvestors = await new Promise((resolve, reject) => {
      db.query(
       ...
       AND (investor_type = 'current' OR investor_type = 'converted' or investor_type = 'previous'  or investor_type = 'warrant not exercised')
       ORDER BY id ASC`,
        [round.id, company_id],
        (err, results) => {
          if (err) reject(err);
    ...
    previousWarrantsTotalShares = previousWarrantsList.reduce(
      (sum, warrant) => sum + (warrant.shares || 0),
      0,
    );
  }
  let totalConvertedWarrantShares = 0;
  let convertedWarrantResults = [];
  try {
    ...
    currentRoundWarrants.forEach((warrant, index) => {
      let investorName = "Warrant Holder";
      let investorEmail = "";
      let investorPhone = "";
      let investorFirstName = "";
      let investorLastName = "";

      if (roundInvestorsList && roundInvestorsList.length > 0) {
        const investor = roundInvestorsList[0];
        investorFirstName = "";
        investorLastName = "";
        investorName = "Warrant Holder";
        investorEmail = "";
        investorPhone = "";
      }

      currentRoundWarrantsList.push({
        type: "warrant",
        name: investorName,
        investor_details: {
        ...
        shares: parseFloat(warrant.shares) || 0,
        investor_type: "warrant not exercised",
        investment: 0,
        share_price: 0,
    ...
    currentRoundWarrantsTotalShares = currentRoundWarrantsList.reduce(
      (sum, warrant) => sum + (warrant.shares || 0),
      0,
    );
  } catch (error) {
  ...
  // ==================== CONVERSION LOGIC FOR SAFE AND CONVERTIBLE NOTES ====================
  ...
  let preconversion = [];
  if (previousRounds.length > 0 && latestRound) {
    preconversion = await getPreviousRoundsForCompanyForConversion(
      company_id,
      latestRound?.id,
    );
  }
  ...
  for (const investor of preconversion) {
    if (alreadyConvertedIds.includes(parseInt(investor.round_id))) {
      continue;
    }
    ...
    const convertedShares = principalPlusInterest / conversionPrice;
    ...
    conversionDetails.push({
      pending_instrument_id: investor.id,
      original_round_id: investor.round_id,
      ...
      convertedSharesRaw: convertedShares,
      investor_name:
    ...
    try {
      ...
      pendingItems.forEach((item) => {
        previousPendingSafes.push({
          ...
          potential_shares: parseFloat(item.potential_shares) || 0,
  ...
  // ==================== CALCULATION BASED ON ROUND TYPE (NO ROUNDING) ====================
  ...
  let totalWarrantValue = 0;

  var ppreMoneyTotalSharesCalc = totalPreMoneyShares;
  var seriesASharePrice = preMoneyVal / ppreMoneyTotalSharesCalc;
  var newInvestorSharesBeforeOption = roundSizeVal / seriesASharePrice;
  ...
  if (hasInvestmentRoundBefore && previousRounds.length > 0) {
    ...
    // ✅ FIX: Calculate founders + seed shares (excluding option pool)
    const foundersAndSeedShares = round0Shares + previousInvestorsTotalShares;

    // ✅ FIX: Series A ownership percentage (from investment / post-money)
    const postMoneyValuationTemp = preMoneyVal + roundSizeVal;
    const seriesAOwnershipPercent =
      (roundSizeVal / postMoneyValuationTemp) * 100;

    // ✅ FIX: Founders + Seed ownership percentage
    const foundersAndSeedOwnershipPercent =
      100 - postMoneyPoolTarget - seriesAOwnershipPercent;

    // ✅ FIX: Total post-money shares (Email formula)
    const totalPostSharesCalc =
      foundersAndSeedShares / (foundersAndSeedOwnershipPercent / 100);

    // ✅ FIX: Total new shares in Series A
    const totalNewShares = totalPostSharesCalc - preMoneyTotalSharesCalc;
    console.log(totalPostSharesCalc, "totalPostSharesCalc");
    // ✅ FIX: Target option pool shares
    const targetOptionShares =
      (postMoneyPoolTarget / 100) * totalPostSharesCalc;

    // ✅ FIX: New option pool shares
    newOptionShares = Math.max(
      0,
      targetOptionShares - existingOptionPoolShares,
    );

    // ✅ FIX: New Series A investor shares
    newInvestorShares = totalNewShares - newOptionShares;
    console.log(newInvestorShares, "newInvestorShares");
    // ✅ FIX: Share price
    const sharesForPrice = preMoneyTotalSharesCalc + newOptionShares;
    updatedSharePrice = preMoneyVal / sharesForPrice;
    // ✅ FIX: Total post shares
    totalPostShares = totalPostSharesCalc;

    // Current Round Warrants
    try {
      const currentRoundWarrants = await new Promise((resolve, reject) => {
        db.query(
          `SELECT w.* 
      ...
      if (currentRoundWarrants && currentRoundWarrants.length > 0) {
        for (const warrant of currentRoundWarrants) {
          const baseInvestor = {
            ...
            convertedShares: newInvestorShares,
          };
    ...
    // Previous Round Warrants Calculation
    const warrantsByRound = {};
    for (const warrant of previousWarrantsList) {
      const originalRoundId = warrant.round_id;
      if (!warrantsByRound[originalRoundId]) {
        warrantsByRound[originalRoundId] = {
          round_id: originalRoundId,
          warrants: [],
          convertedSharesForRound: 0,
        };
      }
      warrantsByRound[originalRoundId].warrants.push(warrant);
    }

    for (const conv of conversionDetails) {
      const originalRoundId = conv.original_round_id;
      if (warrantsByRound[originalRoundId]) {
        warrantsByRound[originalRoundId].convertedSharesForRound =
          conv.convertedShares;
      }
    }

    for (const roundId in warrantsByRound) {
      const roundData = warrantsByRound[roundId];
      const convertedSharesForThisRound = roundData.convertedSharesForRound;

      if (convertedSharesForThisRound === 0) {
        continue;
      }

      const originalRound = previousRounds.find((r) => r.id == roundId);

      let warrantSharePrice = seriesASharePrice;

      if (originalRound) {
        if (
          originalRound.share_price &&
          parseFloat(originalRound.share_price) > 0
        ) {
          warrantSharePrice = parseFloat(originalRound.share_price);
        } else if (
          originalRound.conversion_price &&
          parseFloat(originalRound.conversion_price) > 0
        ) {
          warrantSharePrice = parseFloat(originalRound.conversion_price);
        } else if (originalRound.post_money_cap_table) {
          try {
            const postCapTable =
              typeof originalRound.post_money_cap_table === "string"
                ? JSON.parse(originalRound.post_money_cap_table)
                : originalRound.post_money_cap_table;
            if (
              postCapTable.share_price &&
              parseFloat(postCapTable.share_price) > 0
            ) {
              warrantSharePrice = parseFloat(postCapTable.share_price);
            }
          } catch (e) {}
        }
      }

      const completeWarrants = await new Promise((resolve, reject) => {
        const warrantIds = roundData.warrants.map((w) => w.warrant_id);
        db.query(
          `SELECT w.* 
         FROM warrants w
         WHERE w.id IN (?) 
      ...
      for (const warrant of completeWarrants) {
        // Find the conversion detail that matches this round
        const matchingConversion = conversionDetails.find(
          (conv) => conv.original_round_id == roundId,
        );

        if (!matchingConversion) continue;

        // ✅ Use conversion price from the converted instrument instead of share price
        const conversionPrice = matchingConversion.conversion_price;
        const investmentAmount = matchingConversion.investment;
        const convertedShares = convertedSharesForThisRound;

        const baseInvestor = {
          shares: convertedShares,
          convertedShares: convertedShares,
          conversion_price: conversionPrice, // ✅ Pass conversion price
          investment_amount: investmentAmount, // ✅ Pass investment amount
        };
        ...
        convertedWarrantResults.push({
          warrant_id: warrant.id,
          original_round_id: parseInt(roundId),
          converted_shares_base: convertedSharesForThisRound,
          conversion_price_used: conversionPrice,
          investment_amount: investmentAmount,
          warrant_shares: result.warrantShares,
          warrant_price: result.warrantPrice,
          warrant_value: result.warrantValue,
          warrant_type: warrant.warrantType,
          coverage_percentage: warrant.warrant_coverage_percentage_main,
          discount_percentage: warrant.warrant_coverage_percentage,
        });
      }
    }
    console.log(totalWarrantShares, "totalWarrantShares");
    totalPostShares = totalPostShares + totalWarrantShares;
    ...
    for (const conv of conversionDetails) {
      const discountedPrice =
        updatedSharePrice * (1 - conv.discount_rate / 100);
      const capPrice =
      ...
      const convertedShares = principalPlusInterest / conversionPrice;
      const valueAtSeriesA = convertedShares * updatedSharePrice;
    ...
    totalConvertedShares = convertedSharesTotal;
    ...
    console.log("\n📌 CASE 3: first round (no prev investment rounds)");
    ...
    console.log("\n  📊 CALCULATION RESULTS (NO ROUNDING):");
    console.log("     optionPoolShares:", optionPoolShares);
  ...
  console.log(total_shares_befores);
  // ==================== UPDATE MOIC FOR CONVERSION DETAILS ====================
  conversionDetails.forEach((conv) => {
    ...
    conv.percentage = rawPct.toString() + "%";
    conv.percentage_formatted = rawPct.toString() + "%";
    conv.value = ((rawPct * postMoneyValuation) / 100).toString();
    conv.moic =
      conv.investment > 0
        ? (
  ...
  const buildWarrantItem = (warrant, totalShares, valuation, cur) => {
    ...
    return {
      ...
      percentage_formatted: rawPercentage.toString() + "%",
      value: ((rawPercentage * valuation) / 100).toString(),
      ...
      instrument_type: warrant.instrument_type,
  ...
  // ==================== PREPARE INVESTORS LIST WITH SHARES (NO ROUNDING) ====================
  const totalInvestmentAmount = investorsList.reduce(
    (sum, inv) => sum + (parseFloat(inv.amount) || 0),
    0,
  );

  const investorsWithShares = investorsList.map((inv, idx) => {
    const amount = parseFloat(inv.amount) || 0;
    const proportion =
      totalInvestmentAmount > 0 ? amount / totalInvestmentAmount : 0;
    const shares = proportion * (seriesAInvestorShares || newInvestorShares);
    ...
    return {
      ...
      type: "investor",
      is_new_investment: true,
      percentage: exactPercentage.toString() + "%",
    ...
    founders: {
      list: founderList.map((f) => {
        ...
        return {
          ...
          percentage_raw: rawPercentage,
          percentage: rawPercentage.toString() + "%",
          value: ((rawPercentage * preMoneyVal) / 100).toString(),
        };
      }),
      roundName: round0Name,
      share_class_type: round0Shareclassstype,
      ...
      total_percentage:
        ((round0Shares / preMoneyTotalSharesCalc) * 100).toString() + "%",
      total_value: (
      ...
      previousInvestorsList.length > 0
        ? (() => {
            ...
            return {
              ...
              percentage: groupRawPercentage.toString() + "%",
              total_value: (
                (groupRawPercentage * preMoneyVal) /
                100
              ).toString(),
    ...
    warrants:
      previousWarrantsList.length > 0
        ? {
            ...
            percentage_raw:
              (previousWarrantsTotalShares / preMoneyTotalSharesCalc) * 100,
            percentage:
              (
                (previousWarrantsTotalShares / preMoneyTotalSharesCalc) *
                100
              ).toString() + "%",
            total_value: (
              ((previousWarrantsTotalShares / preMoneyTotalSharesCalc) *
            ...
            items: previousWarrantsList.map((w) =>
              buildWarrantItem(
                ...
                preMoneyTotalSharesCalc,
                preMoneyVal,
                currentRound?.currency || "USD",
              ),
            ),
          }
        : null,
    pre_money_valuation: preMoneyVal,
    share_price: (preMoneyVal / preMoneyTotalSharesCalc).toString(),
    items: [
      ...founderList.map((f) => {
        const ownership = f.shares / preMoneyTotalSharesCalc;
        const rawPercentage = ownership * 100;
  ...
  const postMoneyCapTable = {
    ...
    founders: {
      list: founderList.map((f) => {
        ...
        return {
          ...
          percentage: rawPercentage.toString() + "%",
          percentage_formatted: rawPercentage.toString() + "%",
          value: ((rawPercentage * postMoneyValuation) / 100).toString(),
          shareClassType: f.shareClassType || "Common Shares",
          instrumentType: f.instrumentType || "Common Stock",
      ...
      total_percentage:
        ((round0Shares / totalPostShares) * 100).toString() + "%",
      total_value: (
      ...
      previousInvestorsList.length > 0
        ? (() => {
            ...
            return {
              ...
              percentage_raw: groupRawPercentage,
              percentage: groupRawPercentage.toString() + "%",
              percentage_formatted: groupRawPercentage.toString() + "%",
              total_value: (
              ...
              items: previousInvestorsList.map((inv) => {
                ...
                return {
                  ...
                  percentage_formatted: rawPercentage.toString() + "%",
                  value: (
                    (rawPercentage * postMoneyValuation) /
                    100
                  ).toString(),
                };
              }),
    ...
    investors: (() => {
      ...
      return {
        name:
          hasInvestmentRoundBefore && previousRounds.length > 0
        ...
        shareClassType: currentRound.shareClassType || "Common",
        instrumentType: currentRound.instrumentType || "Common Stock",
        roundName: currentRound.nameOfRound || "Current Round",
    ...
    option_pool: (() => {
      const opSharesFinalCalc =
        existingOptionPoolShares +
        (optionPoolShares || 0) +
        (newOptionShares || 0);
      const ownership = opSharesFinalCalc / totalPostShares;
      const rawPercentage = ownership * 100;
      return {
        shares: opSharesFinalCalc,
        existing_shares: existingOptionPoolShares + (optionPoolShares || 0),
        new_shares: newOptionShares || 0,
        total: opSharesFinalCalc,
        percentage_raw: rawPercentage,
        percentage: rawPercentage.toString() + "%",
        percentage_formatted: rawPercentage.toString() + "%",
        value: ((rawPercentage * postMoneyValuation) / 100).toString(),
    ...
    warrants:
      currentRoundWarrantsList.length > 0
        ? {
            name: "Warrants",
            total_shares: currentRoundWarrantsTotalShares,
            new_shares: currentRoundWarrantsTotalShares,
            total: currentRoundWarrantsTotalShares,
            percentage_raw:
              (currentRoundWarrantsTotalShares / totalPostShares) * 100,
            percentage:
              (
                (currentRoundWarrantsTotalShares / totalPostShares) *
                100
              ).toString() + "%",
            ...
            items: currentRoundWarrantsList.map((w) =>
              buildWarrantItem(
                w,
                totalPostShares,
                postMoneyValuation,
    ...
    items: [
      ...
      ...currentRoundWarrantsList.map((w) =>
        buildWarrantItem(
          w,
          totalPostShares,
          postMoneyValuation,
      ...
      ...(totalConvertedShares > 0
        ? conversionDetails.map((conv) => ({
            ...
            new_shares: conv.convertedShares,
            total: conv.convertedShares,
            existing_shares: conv.convertedShares,
            round_id_ref: conv.round_id_ref,
            is_converted: true,
            percentage_raw: conv.percentage_raw,
            percentage: conv.percentage,
    ...
    share_price: (postMoneyValuation / totalPostShares).toString(),
    roundName: currentRound.nameOfRound,
    roundId: currentRound.id,
    round_id: currentRound.id,
  ...
  // Database update with EXACT values (NO ROUNDING)
  const dbUpdateData = {
    ...
    share_price: updatedSharePrice.toString(),
    issuedshares: (newInvestorShares + (newOptionShares || 0)).toString(),
    investorPostMoney: ((newInvestorShares / totalPostShares) * 100).toString(),
  ...
  return new Promise((resolve, reject) => {
    connection.query(
      ...
      [
        ...
        companyId,
        type,
        parseFloat(optionPool.shares) || 0,
        parseFloat(optionPool.existing_shares) || 0, // ✅ ?? not || (0 valid hai)
        parseFloat(optionPool.new_shares) || 0, // ✅ ?? not ||
        parseFloat(optionPool.total || optionPool.shares) || 0,
        percentage_numeric, // ✅ from item directly
        percentage_formatted, // ✅ from item directly
        value, // ✅ from item directly — no recalculation
  ...
  return new Promise((resolve, reject) => {
    connection.query(
      ...
      [
        ...
        parseFloat(inv.total || inv.shares) || 0,
...
// Function to get warrant data from warrants table
async function getWarrantDataByInvestor(connection, roundId, inv) {
  return new Promise((resolve, reject) => {
    connection.query(
...
// Function to calculate warrant values from warrant data
async function calculateWarrantFromData(warrantData, inv) {
  let potentialShares = 0;
  let adjustedPrice = 0;
  let finalValue = 0;
  let calculationMethod = "";

  // Get existing shares from inv (investor's current shares)
  const existingShares = parseFloat(inv.shares) || 0;
  const sharePrice = parseFloat(inv.share_price) || 0;
  const coveragePercentage =
    parseFloat(warrantData.warrant_coverage_percentage_main) || 0;

  if (warrantData.warrantType === "percentage") {
    // Percentage based warrant with discount
    const discountPercentage =
      parseFloat(warrantData.warrant_coverage_percentage) || 0;
  ...
  } else if (warrantData.warrantType === "fixed") {
    // Fixed price warrant (no discount)
    const fixedWarrantPrice = parseFloat(warrantData.warrant_fixed_shares) || 0;
...
// Function to check if investor has warrant in warrants table
async function checkInvestorWarrant(connection, roundId, inv) {
  return new Promise((resolve, reject) => {
...
// Function to check if warrant is expired
async function isWarrantExpired(warrant) {
  if (!warrant.expiration_date) return false; // No expiry date

  // Convert expiration_date to Date object
  let expiryDate;

  // Handle different date formats
  if (typeof warrant.expiration_date === "string") {
    // Format: '02/April/2026' or '2026-04-02'
    if (warrant.expiration_date.includes("/")) {
      const [day, month, year] = warrant.expiration_date.split("/");
      expiryDate = new Date(`${month} ${day}, ${year}`);
    } else {
      expiryDate = new Date(warrant.expiration_date);
    }
  } else {
    expiryDate = new Date(warrant.expiration_date);
  }
...
async function calculateWarrantFromInvestorData(
  ...
  if (warrant.warrantType === "percentage") {
    ...
    const discountDecimal = 1 - discountPercentage / 100; // 0.85

    // ✅ Step 1: Adjusted Price = Conversion Price × (1 - Discount%)
    ...
    if (discountPercentage === 0) {
      potentialShares = convertedShares * coverageDecimal; // 3,267.97
    } else {
      //potentialShares = totalSharesAtDiscountedPrice * coverageDecimal;
      potentialShares = convertedShares * coverageDecimal;
    }
    ...
    // ✅ Step 1: Adjusted Price = Fixed Price
  ...
  return {
    ...
    discountPercentage: warrant.warrant_coverage_percentage || 0,
  ...
  return new Promise((resolve, reject) => {
    connection.query(
      ...
      [
        ...
        parseFloat(inv.shares) || 0,
        parseFloat(inv.new_shares || inv.shares) || 0,
        parseFloat(inv.total || inv.shares) || 0,
...
async function insertWarrantExercised(
  connection,
  roundId,
  companyId,
  inv,
  ...
  // If warrant exists, calculate and insert warrant entry first
...
// Function to insert warrant as separate entry in round_investors
async function insertWarrantEntry(
  connection,
  roundId,
  companyId,
  inv,
  ...
  return new Promise((resolve, reject) => {
    ...
    // ✅ Step 1: INSERT new warrant record
    const insertQuery = `
      INSERT INTO round_investors 
    ...
    connection.query(insertQuery, values, (insertErr, insertResult) => {
      ...
      const roundInvestorId = insertResult.insertId; // Get the inserted round_investor ID

      // ✅ Step 2: INSERT into warrant_conversion_share_record with round_investor_id
      const warrantShareRecordQuery = `
        INSERT INTO warrant_conversion_share_record 
        (company_id, warrant_id, round_id, round_investor_id, warrant_share, main_shares, created_at)
        VALUES (?, ?, ?, ?, ?, ?, NOW())
      `;

      const warrantShareValues = [
        companyId,
        warrant.id,
        roundId,
        roundInvestorId, // Added round_investor_id
        parseFloat(calculation.potentialShares) || 0, // warrant_share
        parseFloat(calculation.existingShares) || 0, // main_shares
      ];

      connection.query(
        warrantShareRecordQuery,
        warrantShareValues,
        (warrantErr, warrantResult) => {
          if (warrantErr) {
            // Log error but don't reject - main insert was successful
            console.error(
              "Error inserting into warrant_conversion_share_record:",
              warrantErr,
            );
          }

          // ✅ Step 3: DELETE old warrant records (if any)
          const deleteQuery = `
          DELETE FROM round_investors 
          WHERE warrant_id = ? 
            AND round_id = ?
          ...
          connection.query(
            ...
            (deleteErr, deleteResult) => {
              if (deleteErr) {
                ...
                console.error("Error deleting old warrant records:", deleteErr);
              }

              resolve({
                insertResult: insertResult,
                warrantShareRecord: warrantResult,
                roundInvestorId: roundInvestorId,
              });
            },
          );
        },
...
async function insertConvertedInvestorDirect(
  ...
  const potentialShares =
    parseFloat(item.shares) || parseFloat(converted.shares) || 0;
  ...
  return new Promise((resolve, reject) => {
    ...
    connection.query(
      ...
      [
        ...
        parseFloat(conv.shares) || 0, // converted_shares
        parseFloat(conv.conversion_price) || 0, // conversion_price
        isFinite(parseFloat(conv.principal_plus_interest))
          ? parseFloat(conv.principal_plus_interest)
          : 0, // principal_plus_interest
...
async function insertCapTableItemDirect(
  ...
  return new Promise((resolve, reject) => {
    const values = [
      ...
      parseFloat(item.new_shares) || 0,
      parseFloat(item.total || item.shares) || 0,
      percentage_numeric, // ✅ from item directly
      percentage_formatted, // ✅ from item directly
      value, // ✅ from item directly — no recalculation
      ...
      parseFloat(item.existing_shares) || 0, // ✅ ?? not || (0 valid hai)
      ...
      connection.beginTransaction(async (err) => {
        ...
        try {
          ...
          const tables = [
            ...
            "warrant_conversion_share_record",
          ];
                ...
                await new Promise((res, rej) => {
                  ...
                  connection.query(
                    ...
                    [
                      ...
                      "0.00%",
                      0,
                      parseFloat(item.potential_shares) || 0,
                      parseFloat(item.conversion_price) || 0,
...
async function handlePreferredEquityCalculation(params, updateFlag = false) {
  ...
  if (hasInvestmentRoundBefore && previousRounds.length > 0) {
    ...
    const newInvestorSharesBeforeOption = roundSizeVal / seriesASharePrice;
    ...
    try {
      ...
      if (currentRoundWarrants && currentRoundWarrants.length > 0) {
        for (const warrant of currentRoundWarrants) {
          const baseInvestor = {
            shares: newInvestorSharesBeforeOption,
            convertedShares: newInvestorSharesBeforeOption,
          };
          const result = await calculateWarrantSharesForOptionPool(
            warrant,
            baseInvestor,
    ...
    const totalSharesAfterOption =
      totalSharesBeforeOption / (1 - targetOptionPercent);
    const targetTotalOptionShares =
    ...
    newInvestorShares = newInvestorSharesBeforeOption;
    ...
    const totalPostSharesRaw =
      preMoneyTotalSharesCalc / (1 - investorPostMoneyOwnership / 100);
    totalPostShares = totalPostSharesRaw;
    ...
    // Step 1: Calculate existing option pool shares
    const optionPoolSharesRaw =
      (round0Shares / (1 - preMoneyPoolPercent / 100)) *
      (preMoneyPoolPercent / 100);
    optionPoolShares = optionPoolSharesRaw;

    // Step 2: Total pre-money shares for price calculation
    const totalPreMoneySharesForPrice = round0Shares + optionPoolShares;

    // Step 3: Series A Share Price
    const seriesASharePrice = preMoneyVal / totalPreMoneySharesForPrice;

    // Step 4: New Investor Shares (Series A) - YEH FIXED HAI, CHANGE NAHI HOTA
    const newInvestorSharesFixed = roundSizeVal / seriesASharePrice;

    // Step 5: Current Round Warrants
    let totalWarrantSharesTemp = 0;
    // Current Round Warrants
    try {
      const currentRoundWarrants = await new Promise((resolve, reject) => {
        db.query(
      ...
      if (currentRoundWarrants && currentRoundWarrants.length > 0) {
        for (const warrant of currentRoundWarrants) {
          const baseInvestor = {
            shares: newInvestorSharesFixed,
            convertedShares: newInvestorSharesFixed,
          };
          const result = await calculateWarrantSharesForOptionPool(
            warrant,
            baseInvestor,
    ...
    totalWarrantShares = totalWarrantShares;

    // Step 6: Pre-money total shares with warrants
    preMoneyTotalSharesCalc =
      round0Shares +
      optionPoolShares +
    ...
    // Step 7: Option Pool Top-Up (if target > existing)
    if (postMoneyPoolTarget > preMoneyPoolPercent) {
      const targetOptionPercent = postMoneyPoolTarget / 100; // 20% = 0.20

      // Shares without existing option pool (founders + new investors + warrants)
      const sharesBeforeOptionTopUp =
        round0Shares + newInvestorSharesFixed + totalWarrantShares;

      // Total shares after option pool top-up
      const totalSharesAfterOptionTopUp =
        sharesBeforeOptionTopUp / (1 - targetOptionPercent);

      // New option pool shares needed
      newOptionShares =
        totalSharesAfterOptionTopUp * targetOptionPercent - optionPoolShares;

      // ✅ IMPORTANT: newInvestorShares REMAINS THE SAME
      newInvestorShares = newInvestorSharesFixed; // 37,037 ✅

      // Total post shares (with option pool top-up)
      totalPostShares = totalSharesAfterOptionTopUp;

      // Share price remains the same
      updatedSharePrice = seriesASharePrice; // $10.80 ✅

      // Post-money valuation
      postMoneyValuation = totalPostShares * updatedSharePrice;
    } else {
      // No option pool top-up needed
      newInvestorShares = newInvestorSharesFixed;
      totalPostShares = preMoneyTotalSharesCalc + newInvestorShares;
      updatedSharePrice = seriesASharePrice;
      postMoneyValuation = totalPostShares * updatedSharePrice;
      newOptionShares = 0;
    }
    ...
    total_option_pool = optionPoolShares + newOptionShares;
    total_shares_befores = round0Shares;
  }

  // ==================== EMPLOYEE OPTION POOL CONSOLE ====================

  // ==================== CONVERSION PRICE CONSOLE ====================
  ...
  // Build preMoneyCapTable (same structure but with exact values)
  const preMoneyCapTable = {
    total_shares: preMoneyTotalSharesCalc,
    pending_instruments: previousPendingSafes,
    founders: {
  ...
  const opSharesFinal =
    existingOptionPoolShares + (optionPoolShares || 0) + (newOptionShares || 0);
  const optionPoolOwnership = opSharesFinal / totalPostShares;
  const optionPoolRawPercentage = optionPoolOwnership * 100;
...
async function calculateWarrantSharesForOptionPool(
  warrant,
  baseInvestor,
  sharePriceValue,
  investmentAmount = "",
  ...
  const coverageDecimal = coveragePercentage / 100;
  const sharess =
    parseFloat(baseInvestor.shares) || parseFloat(baseInvestor.convertedShares);
  ...
  if (warrant.warrantType === "percentage") {
    ...
    // ✅ Step 1: Warrant Price = Conversion Price × (1 - Discount%)
    ...
    if (discountPercentage === 0) {
      const totalSharesAtDiscountedPrices = sharePriceValue;
      warrantShares = sharess * coverageDecimal;
    } else {
      // warrantShares = totalSharesAtDiscountedPrice * coverageDecimal; // 3,267.97
      warrantShares = sharess * coverageDecimal; // 3,267.97
    }
    ...
    // ✅ Step 1: Warrant Price = Fixed Price
...
async function handleSafeCalculation(params) {
  ...
  let total_shares_before = params.total_shares_before || round0_shares || 0;
  // ==================== PARSE INSTRUMENT DATA ====================
  let parsedInstrumentData = {};
  if (instrumentData) {
  ...
  const round0Shares = parseFloat(total_shares_before) || 0; // ✅ Changed to parseFloat
  ...
  try {
    const currentRoundWarrants = await new Promise((resolve, reject) => {
      db.query(
        `SELECT w.* 
       FROM warrants w
       WHERE w.roundrecord_id = ? AND w.company_id = ?`,
        [id, company_id],
        (err, results) => {
          if (err) reject(err);
          else resolve(results || []);
    ...
    currentRoundWarrants.forEach((warrant, index) => {
      ...
      if (roundInvestorsList && roundInvestorsList.length > 0) {
        const investor = roundInvestorsList[0];
        investorFirstName = "Warrant Holder";
        investorLastName = "";
        investorName = "Warrant Holder";
        investorEmail = "";
        investorPhone = "";
  ...
  // ==================== STEP 1: PRE-MONEY TOTAL SHARES (NO ROUNDING) ====================
  let preMoneyTotalShares;
  let optionPoolShares;

  if (isFirstInvestmentRound) {
    // ✅ NO ROUNDING - Keep exact decimal
    const optionPoolSharesRaw =
  ...
  // ==================== STEP 4: POST-MONEY ====================
  const postMoneyValuation = preMoneyVal + roundSizeVal;
  const postMoneyTotalShares =
    preMoneyTotalShares + currentRoundWarrantsTotalShares;
  ...
  const pendingInstruments = investorsList.map((inv, index) => {
    ...
    return {
      ...
      total: 0,
      principal,
      potential_shares: potentialShares, // ✅ NO ROUNDING
      conversion_price: conversionPrice,
      discount_rate: discountRate,
      valuation_cap: valuationCap,
      investment: principal,
  ...
  if (pendingInstruments.length === 0) {
    const potentialShares =
      conversionPrice > 0 ? roundSizeVal / conversionPrice : 0;
    ...
    try {
      ...
      pendingRows.forEach((row) => {
        ...
        previousPendingSafes.push({
          ...
          existing_shares: 0,
          total: 0,
          potential_shares: parseFloat(row.potential_shares) || 0, // ✅ parseFloat
  ...
  // ==================== HELPER FUNCTIONS (NO ROUNDING IN INTERNAL CALCULATIONS) ====================
  const buildFounderItem = (f, totalShares, valuation, cur) => {
    ...
    return {
      ...
      value: exactValue,
      value_formatted: `${cur} ${exactValue.toFixed(2)}`,
  ...
  const buildOptionPoolItem = (
    ...
    const exactValue = (pct * valuation) / 100;
    return {
      type: "option_pool",
      founder_code: "O",
      name: "Employee Option Pool",
      ...
      percentage_formatted: pct.toFixed(4) + "%",
      value: exactValue,
      value_formatted: `${cur} ${exactValue.toFixed(2)}`,
      shareClassType: "Option Pool",
      instrumentType: "Options",
  ...
  const buildPrevInvestorItem = (inv, totalShares, valuation, cur) => {
    ...
    const rawPercentage = ownership * 100;
    const exactValue = (rawPercentage * valuation) / 100;
    let investorType = "previous";
    if (inv.investor_type === "warrant") {
  ...
  const postMoneyCapTable = {
    ...
    founders: {
      ...
      total_shares: round0Shares,
      total_percentage_raw: (round0Shares / postMoneyTotalShares) * 100,
      total_percentage:
        ((round0Shares / postMoneyTotalShares) * 100).toFixed(4) + "%",
      total_value:
        ((round0Shares / postMoneyTotalShares) * 100 * preMoneyVal) / 100,
      total_value_formatted: `${currency} ${(((round0Shares / postMoneyTotalShares) * 100 * preMoneyVal) / 100).toFixed(2)}`,
    },
    option_pool: {
      shares: totalOptionPoolShares,
      ...
      new_shares: 0,
      total: totalOptionPoolShares,
      percentage_raw: (totalOptionPoolShares / postMoneyTotalShares) * 100,
      percentage:
        ((totalOptionPoolShares / postMoneyTotalShares) * 100).toFixed(4) + "%",
      value:
        ((totalOptionPoolShares / postMoneyTotalShares) * 100 * preMoneyVal) /
        100,
      value_formatted: `${currency} ${(((totalOptionPoolShares / postMoneyTotalShares) * 100 * preMoneyVal) / 100).toFixed(2)}`,
      shareClassType: "Option Pool",
      instrumentType: "Options",
      roundName: "Option Pool",
      is_option_pool: true,
    ...
    previous_investors:
      previousInvestorsList.length > 0
        ? {
            ...
            total: previousInvestorsTotalShares,
            percentage_raw:
              (previousInvestorsTotalShares / postMoneyTotalShares) * 100,
            percentage:
              (
            ...
            total_value:
              ((previousInvestorsTotalShares / postMoneyTotalShares) *
                ...
                preMoneyVal) /
              100,
            total_value_formatted: `${currency} ${(((previousInvestorsTotalShares / postMoneyTotalShares) * 100 * preMoneyVal) / 100).toFixed(2)}`,
            items: previousInvestorsList.map((inv) => {
    ...
    warrants:
      currentRoundWarrantsList.length > 0
        ? {
            ...
            total: currentRoundWarrantsTotalShares,
            percentage_raw:
              (currentRoundWarrantsTotalShares / postMoneyTotalShares) * 100,
            percentage:
              (
                (currentRoundWarrantsTotalShares / postMoneyTotalShares) *
                100
              ).toFixed(4) + "%",
            total_value:
              ((currentRoundWarrantsTotalShares / postMoneyTotalShares) *
                ...
                preMoneyVal) /
              100,
            total_value_formatted: `${currency} ${(((currentRoundWarrantsTotalShares / postMoneyTotalShares) * 100 * preMoneyVal) / 100).toFixed(2)}`,
            items: currentRoundWarrantsList.map((w) =>
              buildWarrantItem(w, postMoneyTotalShares, preMoneyVal, currency),
            ),
            is_grouped: true,
          }
        : null,
    items: [
      ...
      ...previousInvestorsList.map((inv) =>
        buildPrevInvestorItem(inv, postMoneyTotalShares, preMoneyVal, currency),
      ),
      ...previousWarrantsList.map((w) =>
        buildWarrantItem(w, postMoneyTotalShares, preMoneyVal, currency),
      ),
  ...
  // ==================== DATABASE UPDATE (NO ROUNDING) ====================
  const dbUpdateData = {
    share_price: sharePrice.toString(), // ✅ NO toFixed(4)
    issuedshares: "0",
    investorPostMoney: investorPostMoneyOwnership.toString(), // ✅ NO toFixed(2)
    pre_money: preMoneyVal.toString(),
    post_money: postMoneyValuation.toString(),
    roundsize: roundSizeVal.toString(),
    conversion_shares: conversionShares.toString(), // ✅ Exact value
    total_converted_shares: "0",
    option_pool_shares: totalOptionPoolShares.toString(),
    option_pool_percentage: optionPoolPercent.toString(),
...
function updateRoundCalculationsSafeConvertibleNote(params) {
  ...
  const updateValues = [
    ...
    (parseFloat(new_investor_shares) || 0).toString(), // issuedshares
    parseFloat(conversion_price || 0).toFixed(4), // conversion_price
    (parseFloat(conversion_shares) || 0).toString(), // conversion_shares
    (parseFloat(option_pool_shares) || 0).toString(), // option_pool_shares
    (parseFloat(total_shares_before) || 0).toString(), // total_shares_before
    (parseFloat(total_shares_after) || 0).toString(), // total_shares_after
    (parseFloat(total_converted_shares) || 0).toString(), // total_converted_shares
...
async function handleConvertibleNoteCalculation(params) {
  ...
  let effectiveRoundId = id;
  let effectiveRoundName =
    currentRound?.nameOfRound || "Convertible Note Round";
  let effectiveShareClassType =
    currentRound?.shareClassType || "Convertible Note";
  ...
  try {
    ...
    currentRoundWarrants.forEach((warrant, index) => {
      ...
      currentRoundWarrantsList.push({
        ...
        investment: 0,
        share_price: 0,
        is_previous: false, // ✅ Set to false for current round
        is_warrant: true,
        is_new_investment: true, // ✅ Set to true for current round
        share_class_type: null,
        instrument_type: null,
        round_name: currentRound?.nameOfRound || "",
        round_id: id,
    ...
    const roundInvestors = await new Promise((resolve, reject) => {
      db.query(
        `SELECT ri.*, w.id as warrant_id 
           ...
           LEFT JOIN warrants w ON w.id = ri.warrant_id
           WHERE ri.round_id = ? AND ri.company_id = ? AND ri.cap_table_type = 'post' 
           AND (ri.investor_type = 'current' OR ri.investor_type = 'converted' or ri.investor_type = 'previous'  or ri.investor_type = 'warrant not exercised')
           ORDER BY ri.id ASC`,
        [round.id, company_id],
        (err, results) => {
          if (err) reject(err);
  ...
  const pendingInstruments = investorsList.map((inv, index) => {
    ...
    const potentialShares =
      conversionPrice > 0 ? totalConversionAmount / conversionPrice : 0;
    ...
    return {
      ...
      total: 0,
      principal,
      shareClassType: effectiveShareClassType,
      round_name: currentRound?.nameOfRound || "",
      roundId: effectiveRoundId,
      interest_rate: interestRate,
      years: years,
      interest_accrued: interestAccrued,
      total_conversion_amount: totalConversionAmount,
      potential_shares: potentialShares, // ✅ NO ROUNDING
      conversion_price: conversionPrice,
      discount_rate: discountRate,
      valuation_cap: valuationCap,
  ...
  const buildWarrantItem = (warrant, totalShares, valuation, cur) => {
    ...
    return {
      ...
      investment_amount: warrant.investment,
      share_price: warrant.share_price,
      // ✅ Correct flags based on warrant type
      is_previous: isPreviousWarrant, // true for previous round warrants
      is_warrant: true, // always true for warrant items
      is_new_investment: isCurrentWarrant, // true only for current round warrants
      is_converted: false,
      investor_type: investorType,
      share_class_type: warrant.share_class_type,
      instrument_type: warrant.instrument_type,
  ...
  const postMoneyCapTable = {
    ...
    founders: {
      list: founderList.map((f) => {
        const ownership = f.shares / postMoneyTotalShares;
        const rawPercentage = ownership * 100;
        const exactValue = (rawPercentage * postMoneyValuation) / 100;
        return {
          ...f,
          roundName: currentRound?.nameOfRound,
          share_class_type: round0Shareclassstype,
      ...
      total_value:
        ...
        100,
      total_value_formatted: `${currency} ${(((round0Shares / postMoneyTotalShares) * 100 * postMoneyValuation) / 100).toFixed(2)}`,
    },
    option_pool: {
      shares: totalOptionPoolShares,
      existing_shares: totalOptionPoolShares,
      ...
      value:
        ((totalOptionPoolShares / postMoneyTotalShares) *
          ...
          postMoneyValuation) /
        100,
      value_formatted: `${currency} ${(((totalOptionPoolShares / postMoneyTotalShares) * 100 * postMoneyValuation) / 100).toFixed(2)}`,
      shareClassType: "Option Pool",
      instrumentType: "Options",
      roundName: "Option Pool",
      is_option_pool: true,
        ...
        ? {
            ...
            total_value:
              ((previousInvestorsTotalShares / postMoneyTotalShares) *
                ...
                postMoneyValuation) /
              100,
            total_value_formatted: `${currency} ${(((previousInvestorsTotalShares / postMoneyTotalShares) * 100 * postMoneyValuation) / 100).toFixed(2)}`,
            items: previousInvestorsList.map((inv) => {
              const ownership = inv.shares / postMoneyTotalShares;
              const rawPercentage = ownership * 100;
              const exactValue = (rawPercentage * postMoneyValuation) / 100;
        ...
        ? {
            ...
            total_value:
              ((currentRoundWarrantsTotalShares / postMoneyTotalShares) *
                ...
                postMoneyValuation) /
              100,
            total_value_formatted: `${currency} ${(((currentRoundWarrantsTotalShares / postMoneyTotalShares) * 100 * postMoneyValuation) / 100).toFixed(2)}`,
            items: currentRoundWarrantsList.map((w) =>
              buildWarrantItem(
                w,
                postMoneyTotalShares,
    ...
    items: [
      ...
      ...previousWarrantsList.map((w) =>
        buildWarrantItem(w, postMoneyTotalShares, postMoneyValuation, currency),
      ),
      ...currentRoundWarrantsList.map((w) =>
        buildWarrantItem(w, postMoneyTotalShares, postMoneyValuation, currency),
      ),
...
exports.getInvestorCapitalMotionlist = (req, res) => {
  ...
  if (!investor_id) {
    return res.status(400).json({ message: "investor_id is required" });
  }

  const query = `
    SELECT 
      roundrecord.*,
      sharerecordround.id AS share_record_id,
      sharerecordround.sent_date,
      sharerecordround.investor_id,
      sharerecordround.company_id AS shared_by_company_id,
      company.company_name,
      company.company_email,
      company.company_country,
      company.company_city,
      company.company_logo,
      company_investor.archived_status,
      company_investor.joinstatus,
      company_investor.id as company_investor_id
    FROM sharerecordround
    JOIN roundrecord 
      ON roundrecord.id = sharerecordround.roundrecord_id
    LEFT JOIN company 
      ON company.id = sharerecordround.company_id
    LEFT JOIN company_investor 
      ON company_investor.investor_id = sharerecordround.investor_id 
      AND company_investor.company_id = sharerecordround.company_id
    WHERE sharerecordround.investor_id = ?
      AND (company_investor.archived_status IS NULL OR company_investor.archived_status = 'No')
  ...
  db.query(query, [investor_id], (err, results) => {
    ...
    const baseUrl = process.env.API_BASE_URL || "https://capavate.com/api";

    const enrichedResults = results.map((item) => {
      let companyLogoUrl = null;

      if (item.company_logo) {
        const pathname = `upload/docs/doc_${item.shared_by_company_id}`;
        companyLogoUrl = `${baseUrl}/${pathname}/company_profile/${item.company_logo}`;
      }

      return {
        ...item,
        company_logo_url: companyLogoUrl, // ✅ Add full URL
      };
    });
...
exports.getRoundCapTableSingleRecord = (req, res) => {
  ...
  db.query(
    ...
    (err, roundResults) => {
      ...
      // ✅ NEW: Reusable currency cash calculator with warrantType check
      const calcWarrantCurrencyCash = (
        warrantShares,
        strikePrice,
        discountPct,
        warrantType,
      ) => {
        const shares = toNumber(warrantShares);
        const price = toNumber(strikePrice);
        const discount = toNumber(discountPct);
        const type = warrantType || "percentage";

        if (type === "fixed") {
          // Fixed: shares × strike price only (no discount)
          return shares * price;
        } else {
                              ...
                              db.query(
                                ...
                                (err, pendingInstruments) => {
                                  ...
                                  const getPreWarrantsQuery = `
                                    SELECT ri.*, w.expiration_date, w.id as warrant_id,w.warrant_fixed_shares,w.warrant_coverage_percentage,w.warrant_coverage_percentage_main
                                    FROM round_investors ri
                                    LEFT JOIN warrants w ON w.id = ri.warrant_id
                                    WHERE ri.round_id = ? 
                                      AND ri.company_id = ? 
                                    ...
                                    (preWarrantErr, preWarrantResults) => {
                                      ...
                                      db.query(
                                        ...
                                        (
                                          ...
                                          // ✅ postWarrantGroups with discount fields
                                          ...
                                          validPostWarrants.forEach((i) => {
                                            ...
                                            if (!postWarrantGroups[key]) {
                                              postWarrantGroups[key] = {
                                                ...
                                                warrant_coverage_percentage:
                                                  toNumber(
                                                    i.warrant_coverage_percentage,
                                                  ),
                                                warrant_coverage_percentage_main:
                                                  toNumber(
                                                    i.warrant_coverage_percentage_main,
                                                  ),
                                                warrantType: i.warrantType,
                                              };
                                            }
                                          ...
                                          const buildGroupItem = (
                                            ...
                                            // ✅ FIXED: Group currency_cash with fixed/percentage warrantType
                                            const groupCurrencyCash = isWarrant
                                              ? group.items.reduce((sum, i) => {
                                                  return (
                                                    sum +
                                                    calcWarrantCurrencyCash(
                                                      i.total_shares,
                                                      i.share_price,
                                                      i.warrant_coverage_percentage,
                                                      i.warrantType,
                                            ...
                                            return {
                                              ...
                                              is_new_investment: isCurrent,
                                              is_converted: isConverted,
                                              is_warrant: isWarrant,
                                              // ✅ currency_cash at group level
                                              currency_cash: isWarrant
                                                ? parseFloat(
                                                    groupCurrencyCash.toFixed(
                                                      4,
                                                    ),
                                                  )
                                                : 0,
                                              investor_details: group.items.map(
                                                (i) => {
                                                  ...
                                                  const itemIsWarrant =
                                                    ...
                                                    i.investor_type ===
                                                      "warrant not exercised";
                                                  const itemIsPrevious =
                                                    i.investor_type ===
                                                    "previous";
                                                  const itemIsCurrent =
                                                    i.investor_type ===
                                                    "current";
                                                  const itemIsConverted =
                                                    i.investor_type ===
                                                    "converted";
                                                  ...
                                                  // ✅ FIXED: Individual currency_cash with fixed/percentage warrantType
                                                  const currencyCash =
                                                    itemIsWarrant
                                                      ? calcWarrantCurrencyCash(
                                                          i.total_shares,
                                                  ...
                                                  return {
                                                    ...
                                                    round_id_ref:
                                                      i.round_id_ref,
                                                    is_previous: itemIsPrevious,
                                                    is_new_investment:
                                                      itemIsCurrent,
                                                    is_converted:
                                                      itemIsConverted,
                                                    is_warrant: itemIsWarrant,
                                                    investor_details:
                                                      parseDetails(
                                                        i.investor_details,
                                                      ),
                                                    ...
                                                    warrantType: i.warrantType,
                                                    currency_cash: parseFloat(
                                                      currencyCash.toFixed(4),
                                                    ),
                                                  };
                                                },
                                              ),
...
exports.getPreviousRoundForAutoFill = (req, res) => {
  ...
  db.query(getPreviousRoundQuery, params, (err, results) => {
    ...
    const getCumulativePoolQuery = `
      ...
      FROM roundrecord 
      WHERE company_id = ? 
        ${current_round_id ? "AND id < ?" : ""} ORDER BY id DESC 
    LIMIT 1
    `;
...
const createWarrantDataInternal = (warrantData) => {
  return new Promise((resolve, reject) => {
    const {
      roundrecord_id,
      company_id,
    ...
    // Function to format date to YYYY-MM-DD for MySQL
    const formatDateForMySQL = (dateStr) => {
      ...
      // If in DD/Month/YYYY format (e.g., 03/April/2026)
    ...
    // Format dates
    const formattedIssuedDate = formatDateForMySQL(issued_date);
    ...
    const values = [
      warrant_coverage_percentage_main || null,
      warrantType || null,
      warrant_fixed_shares || null,
      roundrecord_id,
      company_id,
      investor_id || 0,
      warrant_coverage_percentage || 0,
...
exports.getroundManagementAcklnowlegment = (req, res) => {
  ...
  const checkRoundZeroQuery = `SELECT id FROM roundrecord WHERE company_id = ? AND round_type = 'Round 0' LIMIT 1`;

  db.query(checkRoundZeroQuery, [company_id], (err, roundZeroResult) => {
    if (err) {
      return res.status(500).json({
        success: false,
        message: "Database query error",
    ...
    if (roundZeroResult.length > 0) {
      ...
      const query = `SELECT * FROM round_acknowlegment WHERE company_id = ?`;
      ...
      db.query(query, [company_id], (err, results) => {
        ...
        res.status(200).json({
          success: true,
          message: "",
          results: results, // Return array (empty if no records)
        });
      });
    } else {
      // No Round 0 exists - return test data or empty array
      var ackResults = [{ email: "test@gmail.com" }];
      res.status(200).json({
        success: true,
        message: "",
        results: ackResults || [],
...
exports.getRoundwarrant = (req, res) => {
  const { company_id, round_id } = req.body;

  if (!company_id || !round_id) {
    return res.status(400).json({
      success: false,
      message: "company_id and round_id are required",
    });
  }
