      ...
      try {
        ...
        if (companyIds.length > 0) {
          ...
          await connection
            .promise()
            .query(
              `DELETE FROM company_add_investors WHERE company_id IN (${placeholders})`,
              companyIds,
            );
          ...
          await connection
            .promise()
            .query(
              `DELETE FROM  investors_warrants WHERE company_id IN (${placeholders})`,
              companyIds,
            );
          await connection
            .promise()
            .query(
              `DELETE FROM round_cap_table_items WHERE company_id IN (${placeholders})`,
              companyIds,
            );
          await connection
            .promise()
            .query(
              `DELETE FROM 	round_conversions WHERE company_id IN (${placeholders})`,
              companyIds,
            );
          await connection
            .promise()
            .query(
              `DELETE FROM 	round_founders WHERE company_id IN (${placeholders})`,
              companyIds,
            );
          await connection
            .promise()
            .query(
              `DELETE FROM round_investors WHERE company_id IN (${placeholders})`,
              companyIds,
            );
          await connection
            .promise()
            .query(
              `DELETE FROM round_option_pools WHERE company_id IN (${placeholders})`,
              companyIds,
            );
          await connection
            .promise()
            .query(
              `DELETE FROM round_pending_instruments WHERE company_id IN (${placeholders})`,
              companyIds,
            );
          await connection
            .promise()
            .query(
              `DELETE FROM investor_round_invite_acknowlegment WHERE company_id IN (${placeholders})`,
              companyIds,
            );
          await connection
            .promise()
            .query(
              `DELETE FROM access_logs_company_round WHERE company_id IN (${placeholders})`,
              companyIds,
            );
          await connection
            .promise()
            .query(
              `DELETE FROM access_logs_investor WHERE company_id IN (${placeholders})`,
              companyIds,
            );
          await connection
            .promise()
            .query(
              `DELETE FROM access_logs_sigantory_last_login WHERE company_id IN (${placeholders})`,
              companyIds,
            );
        }
