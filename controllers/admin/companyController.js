...
exports.getallcompnay = (req, res) => {
  db.query(
    `SELECT company.*, users.first_name, users.last_name 
FROM company 
LEFT JOIN users ON users.id = company.user_id 
...
exports.getCompanyTotalRoundInvestor = (req, res) => {
  const company_id = req.body.company_id;
  ...
  // Query for total rounds (excluding Round 0)
  const roundsQuery = `SELECT COUNT(*) as total_rounds FROM roundrecord WHERE round_type != 'Round 0' AND company_id = ?`;

  // Query for total investors
  const investorsQuery = `SELECT COUNT(*) as total_investors FROM company_investor WHERE company_id = ?`;

  // Query for total raised (sum of roundsize)
  const totalRaisedQuery = `SELECT SUM(roundsize) as total_raised FROM roundrecord WHERE company_id = ?`;

  // Execute all three queries in parallel
  Promise.all([
    new Promise((resolve, reject) => {
      db.query(roundsQuery, [company_id], (err, results) => {
        if (err) reject(err);
        else resolve(results[0]?.total_rounds || 0);
      });
    }),
    new Promise((resolve, reject) => {
      db.query(investorsQuery, [company_id], (err, results) => {
        if (err) reject(err);
        else resolve(results[0]?.total_investors || 0);
      });
    }),
    new Promise((resolve, reject) => {
      db.query(totalRaisedQuery, [company_id], (err, results) => {
        if (err) reject(err);
        else resolve(results[0]?.total_raised || 0);
      });
    }),
  ])
    .then(([total_rounds, total_investors, total_raised]) => {
      res.status(200).json({
        success: true,
...
exports.getCompanyAllInvestment = (req, res) => {
  const company_id = req.body.company_id;
  ...
  const query = `
    ...
    FROM company_investor
    LEFT JOIN investor_information ON investor_information.id = company_investor.investor_id
    LEFT JOIN investorrequest_company ON investorrequest_company.investor_id = company_investor.investor_id 
...
exports.getCompanyFundinground = (req, res) => {
  const company_id = req.body.company_id;

  // Rounds + unique investor count + total invested per round
  // COUNT(DISTINCT investor_id) — ek investor multiple invest kare toh count 1 hi hoga
  const query = `
    SELECT 
      r.*,
      COUNT(DISTINCT irc.investor_id)         AS investor_count,
      COALESCE(SUM(irc.investment_amount), 0) AS total_invested
    FROM roundrecord r
    LEFT JOIN investorrequest_company irc 
           ON irc.roundrecord_id = r.id 
          AND irc.company_id = r.company_id
    WHERE r.company_id = ? And r.round_type != 'Round 0'
    GROUP BY r.id
    ORDER BY r.id DESC
  `;
...
exports.getCompanyDocument = (req, res) => {
  const company_id = req.body.company_id;
  const query = `select * from investor_updates where company_id = ? order by id desc`;
