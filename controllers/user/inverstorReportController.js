  ...
  db.query(query, [user_id, 1], (err, results) => {
    ...
    const updatedResults = results.map((doc) => {
      ...
      if (doc.document_name) {
        downloadUrl = `https://blueprintcatalyst.com/backend/api/${pathname}/investor_report/${doc.document_name}`;
      }
    ...
    for (const report of records) {
      for (const email of emails) {
        ...
        db.query(query, [email], async (err, results) => {
          ...
          const url =
            results.length > 0
              ? `https://blueprintcatalyst.com/backend/investor/login`
              : `https://blueprintcatalyst.com/backend/investor/information/${token}`;
          ...
          <tr>
            <td style="background: #efefef; padding: 10px 0; text-align: center;">
              <div style="width: 130px; margin: 0 auto;">
                <img src="https://blueprintcatalyst.com/backend/upload/images/logo.png" alt="Capavate" style="width: 100%;" />
              </div>
            </td>
          </tr>
          <tr>
                ...
                <tr>
                  <td style="padding: 20px;">
                    ...
                    <p style="margin: 0 0 15px 0; font-size: 14px; color: #111;">
                      Your account has been successfully created on <strong>Capavate.com</strong>, powered by Blueprint Catalyst Limited.
                    </p>
                    <p style="margin: 0 0 15px 0; font-size: 14px; color: #111;">
  ...
  db.query(query, [email], (err, results) => {
    ...
    if (results.length > 0) {
      ...
      const updatedResults = filteredResults.map((doc) => ({
        ...doc,
        downloadUrl: `https://blueprintcatalyst.com/backend/api/upload/docs/doc_${doc.user_id}/investor_report/${doc.document_name}`,
      }));
