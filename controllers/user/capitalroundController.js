...
async function handlePreferredEquityCalculation(params, updateFlag = false) {
  ...
  if (hasInvestmentRoundBefore && previousRounds.length > 0) {
    ...
    console.log("📌 PREVIOUS ROUND WARRANTS");
    console.log(
      `   Total Previous Warrants in list: ${previousWarrantsList.length}`,
    );
    ...
    for (const roundId in warrantsByRound) {
      ...
      if (convertedSharesForThisRound === 0) {
        console.log(
          `   ⚠️ Round ${roundId}: No converted shares found, skipping warrants`,
        );
        continue;
      }
      ...
      console.log(`\n   📌 Round ID: ${roundId}`);
      console.log(
        `   Converted Shares for this round: ${convertedSharesForThisRound}`,
      );
      console.log(`   Warrants found in DB: ${completeWarrants.length}`);
      ...
      for (const warrant of completeWarrants) {
        ...
        console.log(
          `      Warrant ${warrant.id}: ${result.warrantShares} shares @ ${result.warrantPrice} = ${result.warrantValue}`,
        );
      }
    }

    console.log(
      `\n📊 TOTAL PREVIOUS ROUND WARRANT SHARES: ${totalWarrantShares}`,
    );
    console.log(totalWarrantShares);
...
async function calculateWarrantSharesForOptionPool(
  ...
  console.log(`   Calculating Warrant ID: ${warrant.id}`);
  console.log(`   Warrant Type: ${warrant.warrantType}`);
  console.log(`   Base Shares: ${baseShares}`);
