...
exports.CreateOrUpdateCapitalRound = (req, res) => {
  ...
  uploadFields(req, res, async (err) => {
    ...
    if (id && id !== "undefined" && id !== null && id !== "") {
      // ---------------- UPDATE CASE ----------------
      // First get existing record to merge files
      db.query(
        ...
        (err, results) => {
          ...
          // Handle files only if new files uploaded
          if (newTermsheetFiles !== null) {
            sql += `, termsheetFile = ?`;
            const allTermsheetFiles = [
              ...(existingTermsheetFiles
                ? JSON.parse(existingTermsheetFiles)
                : []),
              ...newTermsheetFiles,
            ];
      ...
      const sql = `INSERT INTO roundrecord
        (company_id,roundStatus,instrument_type_data,created_by_id,created_by_role,dateroundclosed,nameOfRound, shareClassType, shareclassother, description, instrumentType, customInstrument, roundsize, currency, issuedshares, rights, liquidationpreferences, liquidation, liquidationOther, convertible, convertibleType, voting, termsheetFile, subscriptiondocument, generalnotes, created_at)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`;
...
exports.getcheckCapitalMotionlist = (req, res) => {
  ...
  const query = `SELECT roundrecord.*,sharerecordround.signature_status,sharerecordround.signature,sharerecordround.investor_id,sharerecordround.id as sharerecordround_id, company.company_name FROM roundrecord JOIN company ON company.id = roundrecord.company_id JOIN sharerecordround ON sharerecordround.roundrecord_id = roundrecord.id WHERE roundrecord.id = ? AND sharerecordround.investor_id = ?;`;
