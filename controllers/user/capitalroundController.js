            ...
            <tr>
              <td style="padding:30px;">
                ...
                <div style="text-align:center;margin:30px 0;">
                  <a href="https://blueprintcatalyst.com/investor/dashboard" style="background:#10b981;color:#fff;text-decoration:none;font-size:16px;font-weight:500;padding:12px 40px;border-radius:8px;display:inline-block;">
              ...
              if (isSeriesA) {
                ...
                db.query(
                  ...
                  (err, roundResults) => {
                    ...
                    console.log("✅ Round found:", round.nameOfRound);
         ...
         ORDER BY created_at ASC LIMIT 1`,
             ...
             ORDER BY created_at ASC`,
                          ...
                          (err, previousRounds) => {
                            ...
                            console.log(
                              `✅ Previous rounds found: ${previousRounds.length}`
                            );
                 ...
                 AND ir.request_confirm = 'Yes'`,
                              ...
                              (err, currentInvestors) => {
                                ...
                                console.log(
                                  `✅ Series A investors found: ${currentInvestors.length}`
                                );
                                ...
                                // Step 5: Calculate Series A cap table
                                console.log(
                                  "\n🎯 Starting Series A calculation..."
                                );
                                ...
                                console.log("✅ Series A calculation complete");
                                console.log(
                                  "========== API COMPLETE ==========\n"
                                );
...
function calculateSeriesARoundCapTable(
  ...
  try {
    ...
    console.log(
      `   Founders+Seed: ${(
        totalExistingShares - totalOptionPoolShares
      ).toLocaleString()}`
    );
    ...
    console.log("\n📥 SERIES A INPUTS:");
    console.log(`   Investment: $${investmentSize.toLocaleString()}`);
    console.log(`   Pre-Money: $${preMoneyValuation.toLocaleString()}`);
    console.log(`   Target Pool: ${optionPoolPercentPost}%`);
    ...
    console.log("\n🎯 BASIC CALCULATIONS:");
    console.log(`   Post-Money: $${postMoneyValuation.toLocaleString()}`);
    console.log(`   Investor %: ${investorOwnershipPercent.toFixed(2)}%`);
    ...
    if (needsOptionPoolExpansion) {
      ...
      console.log(
        `   Founders+Seed: ${foundersSeedActual.toFixed(
          2
        )}% (target: ${foundersSeedPercent.toFixed(2)}%)`
      );
      console.log(
        `   Option Pool: ${optionPoolActual.toFixed(
          2
        )}% (target: ${optionPoolPercentPost}%)`
      );
      console.log(
        `   Investors: ${investorActual.toFixed(
          2
        )}% (target: ${investorOwnershipPercent.toFixed(2)}%)`
      );
      console.log(
        `   Total: ${(
          foundersSeedActual +
...
// Helper function to get existing shareholders structure
function getExistingShareholdersStructure(roundZeroFounders, previousRounds) {
  const stakeholders = [];
  let totalShares = 0;
  let optionPoolShares = 0;
  console.log(previousRounds, "kkk");
  // Add Round 0 founders
  ...
  if (previousRounds && previousRounds.length > 0) {
    // For now, we'll assume the last round before Series A is the Seed Round
    // and it has the structure we need
    const seedRound = previousRounds[previousRounds.length - 1];

    if (seedRound && seedRound.capTable && seedRound.capTable.shareholders) {
      seedRound.capTable.shareholders.forEach((sh) => {
        const shares = toNumber(sh.shares, 0);
        console.log(shares, "kll");
        if (shares > 0) {
          if (sh.type === "Options Pool") {
            optionPoolShares += shares;
            stakeholders.push({
              name: sh.name,
              fullName: sh.fullName || sh.name,
              type: "Options Pool",
              shares: shares,
              originalType: "Options Pool",
            });
          } else if (sh.type === "Investor") {
            stakeholders.push({
              name: sh.name,
              fullName: sh.fullName || sh.name,
              type: "Investor",
  ...
  db.query(query, params, (err, results) => {
    ...
    results.forEach((round, index) => {
      ...
      if (round.round_type === "Round 0") {
        ...
        console.log(`  Round 0 Total: ${totalExistingShares}`);
        ...
        console.log(`  Investor shares: ${prevRoundInvestorShares}`);
        console.log(`  Pre-Money Pool: ${prevRoundPreMoneyPool}%`);
        console.log(`  Post-Money Pool: ${prevRoundPostMoneyPool}%`);
        ...
        if (prevRoundPreMoneyPool > 0) {
          ...
          console.log(`  Option pool calculation (PRE-money):`);
          console.log(`    Shares before pool: ${sharesBeforePool}`);
          ...
          if (investorPostMoney > 0 && prevRoundInvestorShares > 0) {
            ...
            console.log(`  Option pool calculation (POST-money):`);
            console.log(`    Total after round: ${totalAfterPrevRound}`);
            console.log(`    Option pool shares: ${optionPoolShares}`);
        ...
        console.log(`  Running total: ${totalExistingShares}`);
