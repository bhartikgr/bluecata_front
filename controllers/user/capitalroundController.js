      ...
      (err, warrants) => {
        ...
        `;
          ...
          db.query(
            ...
            (err2) => {
              if (err2) {
                console.error("Error deleting from round_investors:", err2);
          ...
          db.query(sql, values, async (err) => {
            ...
            if (hasWarrants_preferred === "true") {
              ...
              try {
                ...
                console.error("Error updating/creating warrant:", warrantErr);
      ...
      db.query(sql, values, async (err, result) => {
        ...
        if (hasWarrants_preferred === "true") {
          ...
          try {
            ...
            console.error("Error creating warrant:", warrantErr);
...
async function handleCommonStockCalculation(params, updateFlag = false) {
  ...
  try {
    ...
    // Parse investors list for names
  ...
  const buildWarrantItem = (warrant, totalShares, valuation, cur) => {
    ...
    return {
      ...
      value: ((rawPercentage * valuation) / 100).toFixed(2),
...
async function getWarrantDataByInvestor(connection, roundId, inv) {
  return new Promise((resolve, reject) => {
    connection.query(
      ...
      (err, results) => {
        if (err) {
          console.error("Error fetching warrant data:", err);
          reject(err);
        } else {
...
async function calculateWarrantFromInvestorData(warrant, inv) {
  let potentialShares = 0;
  let adjustedPrice = 0;
  let finalValue = 0;
  let calculationMethod = "";
...
async function saveCapTableData(
  ...
  console.log("🚀 saveCapTableData STARTED with params:", {
    checkround,
    roundId,
    companyId,
    preMoneyTotalShares,
          ...
          if (preTable) {
            ...
            if (convert === false) {
              if (preTable.converted) {
                console.log("  🔄 Saving converted investors (PRE)");
...
async function handlePreferredEquityCalculation(params, updateFlag = false) {
  ...
  console.log("========== OPTION POOL CALCULATION DEBUG ==========");
  ...
  // ========== CONDITION: Check if previousRounds is empty ==========
  const previousRounds = await getPreviousRoundsForCompany(company_id, id);
  const sortedPreviousRounds = [...previousRounds].sort((a, b) => b.id - a.id);
  const latestPreviousRound = sortedPreviousRounds[0];

  // ✅ CONDITION: If previousRounds is empty, set all related variables to empty/default
  ...
  // If previousRounds is empty, override round0Name
  ...
  // ✅ CONDITION: Only fetch previous investors if there are previous rounds
  ...
  // ✅ CONDITION: Only fetch preconversion if there are previous rounds
  let preconversion = [];
  ...
  if (hasInvestmentRoundBefore && previousRounds.length > 0) {
    ...
    console.log("🔄 CONVERSION DETAILS (CASE 1):");
    console.log("   convertedSharesTotal:", convertedSharesTotal);
    console.log("   convertedInvestmentTotal:", convertedInvestmentTotal);
    console.log("   conversionDetails count:", conversionDetails.length);
    if (conversionDetails.length > 0) {
      conversionDetails.forEach((conv, idx) => {
        console.log(`   Conversion ${idx + 1}:`);
        console.log(`      Investor: ${conv.investor_name}`);
        ...
        console.log(`      Discount Rate: ${conv.discount_rate}%`);
        console.log(`      Valuation Cap: ${conv.valuation_cap}`);
        if (conv.interest_rate > 0) {
          console.log(`      Interest Rate: ${conv.interest_rate}%`);
          console.log(`      Interest Accrued: ${conv.interest_accrued}`);
          console.log(
            `      Principal + Interest: ${conv.principal_plus_interest}`,
          );
        }
      });
    }
    ...
    console.log("📊 OPTION POOL CALCULATION (CASE 1):");
    console.log(
      "   newInvestorSharesBeforeOption:",
      newInvestorSharesBeforeOption,
    );
    console.log("   totalSharesBeforeOption:", totalSharesBeforeOption);
    console.log("   targetOptionPercent:", targetOptionPercent);
    console.log("   totalSharesAfterOption:", totalSharesAfterOption);
    console.log("   targetTotalOptionShares:", targetTotalOptionShares);
    console.log("   existingOptionPoolShares:", existingOptionPoolShares);
    console.log("   newOptionShares (issued in this round):", newOptionShares);
    console.log("");
    ...
    console.log("📌 CASE 2: Previous Round is Round 0");
    ...
    console.log("🔄 CONVERSION DETAILS (CASE 2):");
    console.log("   totalConvertedShares:", totalConvertedShares);
    console.log("   totalConvertedInvestment:", totalConvertedInvestment);
    console.log("   conversionDetails count:", conversionDetails.length);
    if (conversionDetails.length > 0) {
      conversionDetails.forEach((conv, idx) => {
        console.log(`   Conversion ${idx + 1}:`);
    ...
    console.log("📌 CASE 3: Previous Rounds Exist (Other)");
    ...
    console.log("🔄 CONVERSION DETAILS (CASE 3):");
    console.log("   totalConvertedShares:", totalConvertedShares);
    console.log("   totalConvertedInvestment:", totalConvertedInvestment);
    console.log("   conversionDetails count:", conversionDetails.length);
    if (conversionDetails.length > 0) {
    ...
    console.log("📌 CASE 4: NO PREVIOUS ROUNDS - First Round Calculation");
    ...
    console.log("🔄 CONVERSION DETAILS (CASE 4):");
    console.log("   totalConvertedShares:", totalConvertedShares);
    console.log("   totalConvertedInvestment:", totalConvertedInvestment);
    console.log("   conversionDetails count:", conversionDetails.length);
    if (conversionDetails.length > 0) {
  ...
  // 📊 FINAL CONVERSION SHARES SUMMARY
  console.log("========== CONVERSION SHARES SUMMARY ==========");
  console.log("📊 TOTAL CONVERTED SHARES:", totalConvertedShares);
  console.log("📊 TOTAL CONVERTED INVESTMENT:", totalConvertedInvestment);
  console.log("📊 NUMBER OF CONVERTED INVESTORS:", conversionDetails.length);
  console.log("");

  if (conversionDetails.length > 0) {
    console.log("📋 DETAILED CONVERSION BREAKDOWN:");
    conversionDetails.forEach((conv, idx) => {
      console.log(`   ${idx + 1}. ${conv.investor_name}`);
      console.log(`      Instrument: ${conv.instrument_type}`);
      console.log(`      Original Investment: ${conv.investment}`);
      console.log(`      Conversion Price: ${conv.conversion_price}`);
      console.log(`      Converted Shares: ${conv.convertedShares}`);
      if (conv.interest_rate > 0) {
        console.log(`      Interest Rate: ${conv.interest_rate}%`);
        console.log(`      Interest Accrued: ${conv.interest_accrued}`);
        console.log(
          `      Total with Interest: ${conv.principal_plus_interest}`,
        );
      }
      console.log(`      Round Reference: ${conv.round_id_ref}`);
    });
  }
  console.log("===============================================");
  console.log("");
  ...
  // 📊 FINAL OWNERSHIP SUMMARY AFTER CONVERSION
  ...
  console.log(
    `   Founders: ${round0Shares} shares (${((round0Shares / totalPostShares) * 100).toFixed(2)}%)`,
  );
  console.log(
    `   Option Pool: ${existingOptionPoolShares + (optionPoolShares || 0) + (newOptionShares || 0)} shares (${(((existingOptionPoolShares + (optionPoolShares || 0) + (newOptionShares || 0)) / totalPostShares) * 100).toFixed(2)}%)`,
  );
  console.log(
    `   Converted Investors: ${totalConvertedShares} shares (${((totalConvertedShares / totalPostShares) * 100).toFixed(2)}%)`,
  );
  console.log(
    `   New Investors: ${newInvestorShares} shares (${((newInvestorShares / totalPostShares) * 100).toFixed(2)}%)`,
  );
  if (currentRoundWarrantsTotalShares > 0) {
    console.log(
      `   Warrants: ${currentRoundWarrantsTotalShares} shares (${((currentRoundWarrantsTotalShares / totalPostShares) * 100).toFixed(2)}%)`,
    );
  }
  console.log("");

  if (conversionDetails.length > 0) {
    console.log("📋 CONVERTED INVESTORS OWNERSHIP:");
    conversionDetails.forEach((conv, idx) => {
      console.log(
        `   ${idx + 1}. ${conv.investor_name}: ${conv.convertedShares} shares (${conv.percentage}) - Value: ${conv.value} - MOIC: ${conv.moic}`,
      );
    });
  }
  console.log("==============================================");
  ...
  console.log("========== FINAL OPTION POOL CALCULATION ==========");
  console.log("📊 OPTION POOL FINAL VALUES:");
  console.log("   existingOptionPoolShares:", existingOptionPoolShares);
  console.log("   optionPoolShares (from pre-money):", optionPoolShares || 0);
  console.log(
    "   newOptionShares (issued in this round):",
    newOptionShares || 0,
  );
  console.log("   TOTAL opShares:", opShares);
  console.log("   totalPostShares:", totalPostShares);
  console.log("   postMoneyValuation:", postMoneyValuation);
  console.log("");

  const optionPoolOwnership = opShares / totalPostShares;
  const optionPoolRawPercentage = optionPoolOwnership * 100;
  const optionPoolValue = (optionPoolRawPercentage * postMoneyValuation) / 100;

  console.log("📊 OPTION POOL PERCENTAGE & VALUE:");
  console.log("   Formula: opShares / totalPostShares × 100");
  console.log(
    `   Calculation: ${opShares} / ${totalPostShares} × 100 = ${optionPoolRawPercentage.toFixed(2)}%`,
  );
  console.log(
    `   Option Pool Percentage: ${optionPoolRawPercentage.toFixed(2)}%`,
  );
  console.log("");
  console.log("   Formula: (Percentage × Post-Money Valuation) / 100");
  console.log(
    `   Calculation: (${optionPoolRawPercentage.toFixed(2)} × ${postMoneyValuation}) / 100 = ${optionPoolValue.toFixed(2)}`,
  );
  console.log(`   Option Pool Value: ${optionPoolValue.toFixed(2)}`);
  console.log("");

  console.log("📊 VERIFICATION WITH EXAMPLE:");
  console.log("   Expected (from your example):");
  console.log("   Employee total shares: 38,461");
  console.log("   Total shares after pool: 192,303");
  console.log("   Employee Ownership %: 20%");
  console.log("   Post-Money Valuation: 2,076,875");
  console.log("   Employee Value: 415,375");
  console.log("");
  console.log("   ACTUAL CALCULATION:");
  console.log(`   Ownership %: ${optionPoolRawPercentage.toFixed(2)}%`);
  console.log(`   Value: ${optionPoolValue.toFixed(2)}`);
  console.log("==========================================");
  console.log("");
  ...
  const postMoneyCapTable = {
    ...
    option_pool: (() => {
      const opSharesFinal =
        ...
        (optionPoolShares || 0) +
        (newOptionShares || 0);
      const ownership = opSharesFinal / totalPostShares;
      const rawPercentage = ownership * 100;

      console.log("📊 POST-MONEY OPTION POOL:");
      console.log("   opSharesFinal:", opSharesFinal);
      console.log("   totalPostShares:", totalPostShares);
      console.log("   rawPercentage:", rawPercentage);
      console.log(
        "   value:",
        ((rawPercentage * postMoneyValuation) / 100).toFixed(2),
      );
      console.log("");

      return {
        shares: opSharesFinal,
        existing_shares: existingOptionPoolShares + (optionPoolShares || 0),
        new_shares: newOptionShares || 0,
        total: opSharesFinal,
        percentage_raw: rawPercentage,
  ...
  try {
    ...
    console.log("✅ Option Pool Calculation Complete");
    console.log(`   Option Pool Shares: ${opShares}`);
    console.log(
      `   Option Pool Percentage: ${optionPoolRawPercentage.toFixed(2)}%`,
    );
    console.log(`   Option Pool Value: ${optionPoolValue.toFixed(2)}`);
    console.log("========================================");
    console.log("");
    console.log("========== CONVERSION SHARES FINAL SUMMARY ==========");
    console.log(`✅ Total Converted Shares: ${totalConvertedShares}`);
    console.log(`✅ Total Converted Investment: ${totalConvertedInvestment}`);
    console.log(
      `✅ Number of Converted Investors: ${conversionDetails.length}`,
    );
    console.log("=====================================================");
...
async function handleSafeCalculation(params) {
  ...
  try {
    ...
    currentRoundWarrants.forEach((warrant, index) => {
      ...
      // Try to get from round_investments first
      if (roundInvestorsList && roundInvestorsList.length > 0) {
        const investor = roundInvestorsList[0];
      ...
      // If still empty, use warrant table data (if stored)
      if (!investorName || investorName === "Warrant Holder") {
        // Check if warrant table has investor details stored
        investorFirstName = "";
        investorLastName = "";
        investorName = "Warrant Holder";
        investorEmail = "";
  ...
  const discountedPrice = sharePrice * (1 - discountRate / 100);
  const capPrice = valuationCap > 0 ? valuationCap / preMoneyTotalShares : "";
  const conversionPrice = Math.min(discountedPrice, capPrice);

  // ==================== STEP 4: POST-MONEY ====================
  const postMoneyValuation = preMoneyVal + roundSizeVal;
  const postMoneyTotalShares =
    preMoneyTotalShares + currentRoundWarrantsTotalShares; // ✅ Add current round warrants
  ...
  const postMoneyCapTable = {
    ...
    // ✅ Add current round warrants to items
    warrants:
      currentRoundWarrantsList.length > 0
        ? {
            name: "Warrants",
...
async function handleConvertibleNoteCalculation(params) {
  ...
  const pendingInstruments = investorsList.map((inv, index) => {
    ...
    return {
      ...
      round_name: currentRound?.nameOfRound || "",
      roundId: effectiveRoundId, // ✅ FIXED: Using effectiveRoundId
      interest_rate: interestRate,
      ...
      value_formatted: "0.00",
      is_pending: true,
      round_id: id, // ✅ FIXED: Use the round id from params
      round_id_ref: effectiveRoundId, // ✅ FIXED
      investor_details: {
        firstName: inv.firstName || "",
        lastName: inv.lastName || "",
        email: inv.email || "",
  ...
  if (pendingInstruments.length === 0) {
    ...
    pendingInstruments.push({
      ...
      shareClassType: "",
      round_name: effectiveRoundName,
      roundId: effectiveRoundId, // ✅ FIXED
      round_id_ref: effectiveRoundId, // ✅ FIXED
      investor_details: {
        firstName: "",
        lastName: "",
                                  ...
                                  db.query(
                                    ...
                                    (preWarrantErr, preWarrantResults) => {
                                      ...
                                      const buildPreWarrantItem = (
                                        ...
                                        existing_shares: 0,
                                        total: warrant.shares,
                                        shares_formatted: fmt(warrant.shares),
                                        ...
                                        value: parseFloat(warrant.value || 0),
                                        value_formatted: money(warrant.value),
                                        ...
                                        is_new_investment: true,
                                        is_converted: false,
                                        investor_type: warrant.investor_type,
                                        share_class_type:
                                          ...
                                          const buildGroupItem = (
                                            ...
                                            investor_details: group.items.map(
                                              (i) => ({
                                                ...
                                                discount_rate:
                                                  parseFloat(i.discount_rate) ||
                                                  0,
                                                valuation_cap:
                                                  parseFloat(i.valuation_cap) ||
                                                  0,
                                                interest_rate:
