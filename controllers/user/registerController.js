        ...
        >
          <tr>
            <td style="background: #efefef; padding: 10px 0; text-align: center;">
              <img src="https://yourdomain.com/logo.png" alt="logo" style="width: 130px;" />
            </td>
          </tr>
          <tr>
            <td style="padding: 20px; color: #111; font-size: 14px;">
              ...
              <p style="margin: 0 0 15px 0;">
                Thank you for registering with <b>Blueprint Catalyst Ltd</b>.
              </p>
              <p style="margin: 0 0 15px 0;">
...
exports.checkUserEmail = async (req, res) => {
  ...
  db.query(checkCompanyQuery, [email], (err, results) => {
    ...
    if (results.length > 0) {
      ...
      var date = new Date();
      const query = `INSERT INTO users (name,email,password,viewpassword,phone_number,activate_account_code,created_at) VALUES (?, ?, ?, ?, ?, ?, ?)`;
      ...
      if (checkdata.activate_account_code === code) {
        if (checkdata.status === "Active") {
          ...
          db.query(updateQuery, [email, code], (err, updateResults) => {
            ...
            // 3️⃣ Send activation success email
            sendEmailAccountActivated(email, results[0].name || "User");
      ...
      <div style="width: 600px; margin: 0 auto; border: 1px solid #e5e7eb; border-radius: 10px; overflow: hidden; background: #fff;">
        <table style="width: 100%; border-collapse: collapse;">
          ...
          <tr>
            <td style="padding: 20px; color: #111; font-size: 14px;">
              <h2>Hello ${fullName},</h2>
              <p>Thank you for registering with <b>Blueprint Catalyst Ltd</b>.</p>
    ...
    db.query(query, [email], async (err, results) => {
      ...
      if (results.length > 0) {
        ...
        if (results[0].status === "Activate") {
          ...
          const updateQuery =
            "UPDATE users SET password = ?, view_password = ? WHERE email = ?";
          db.query(
            updateQuery,
...
exports.companyaddWithSignatory = (req, res) => {
  const { user_id, company_name, signatories, ...rest } = req.body;
  ...
  db.query(
    ...
    (userErr, userResult) => {
      ...
      db.query(
        ...
        INSERT INTO company
        (company_name, user_id, company_color_code, company_industory, phone, company_website, employee_number, year_registration, formally_legally, company_street_address, company_country, company_state, company_city, company_postal_code, descriptionStep4, problemStep4, solutionStep4)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `;
