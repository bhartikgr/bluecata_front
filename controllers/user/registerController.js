        ...
        <table style="width: 100%; border-collapse: collapse;">
          ...
          <tr>
            <td style="padding: 20px; color: #111; font-size: 14px;">
              ...
              <div style="text-align: center; margin: 20px 0;">
                <a href="http://localhost:5000/user/login" style="background:#CC0000; color:#fff; text-decoration:none; font-size:14px; padding:10px 30px; border-radius:10px;">Login Now</a>
              </div>
              <p style="margin: 30px 0 10px 0; font-size: 14px; text-align: center; color: #555;">
                Capavate Powered by <b>Blueprint Catalyst Ltd</b>
              </p>
...
exports.companyaddWithSignatory = (req, res) => {
  ...
  upload(req, res, function (err) {
    ...
    const filename = req.file ? req.file.filename : rows[0]?.articles || null;
              ...
              db.query(
                ...
                (colorErr, colorResults) => {
                  ...
                  const companyInsertQuery = `
                    INSERT INTO company
                    (signatory_acknowledgement,ticker_symbol,selectedExchange,unit_number,isPubliclyTraded,signatory_designation, company_email, state_code, country_code, company_name, user_id, company_color_code, 
                ...
                `;
                  ...
                  db.query(
                    companyInsertQuery,
                    [
                      rest.signatory_acknowledgement || null,
                      rest.ticker_symbol || null,
                      rest.selectedExchange || null,
                      rest.unit_number || null,
                      rest.isPubliclyTraded || null,
                      signatory_acknowledged,
                      company_email,
                      state_code,
                      country_code,
                    ...
                    (err, companyResult) => {
                      ...
                      // Insert signatories
                      // In the signatory insertion section, add validation before sending email
                      if (signatories && signatories.length) {
                        let insertedCount = 0;
                        const signatoryInsertQuery = `
                          INSERT INTO company_signatories
                          (isFounder,company_id, unique_code, user_id, first_name, last_name, signatory_email, linked_in, signatory_phone, signature_role, access_status, invited_by, invited_at)
                          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                        `;
                        ...
                        signatories.forEach((s) => {
                          ...
                          db.query(
                            ...
                            (signErr) => {
                              ...
                              // ✅ Only send email if signatory_email exists
                              if (
                                s.signatory_email &&
                                ...
                                console.log(
                                  `⚠️ Skipping email for signatory - no email provided: ${s.first_name} ${s.last_name}`,
                                );
                              }
                        ...
                        return res.status(200).json({
                          message:
                            "Company added successfully (no signatories)",
                          status: "1",
                          companyId,
                          company_color_code: assignedColor,
                        });
      ...
      <table style="width:100%; border-collapse: collapse;">
        ...
        <tr>
          <td style="padding:25px;">
            ...
            <p style="font-size:14px; color:#111; margin-bottom:15px;">
              <a href="http://localhost:5000/user/dashboard" style=" background: #CC0000;
...
exports.companyProfileUpdate = (req, res) => {
  ...
  upload(req, res, function (err) {
    ...
    const companyValues = [
      req.body.ticker_symbol || null,
      req.body.selectedExchange || null,
      ...
      req.body.isPubliclyTraded || null,
      req.body.unit_number || null,
      // WHERE clause
      company_id,
    ];
        ...
        <table style="width:100%;border-collapse:collapse;">
          ...
          <tr>
            <td style="padding:25px;">
              ...
              <p style="font-size:14px;color:#111;margin-bottom:15px;">
                <a href="http://localhost:5000/authorized-signature" style="background:#CC0000;color:#fff;padding:10px 20px;text-decoration:none;border-radius:5px;font-weight:bold;">Click Here</a>
              </p>
              <p style="font-size:14px;color:#111;margin-bottom:0;">Regards,<br/>Capavate Team</p>
            </td>
          </tr>
      ...
      <table style="width:100%; border-collapse: collapse;">
        ...
        <tr>
          <td style="padding:25px;">
            ...
            <p style="font-size:14px; color:#111; margin-bottom:15px;">
              <a href="http://localhost:5000/authorized-signature" style="background:#CC0000; color:#fff; padding:10px 20px; text-decoration:none; border-radius:5px; font-weight:bold;">
                Click Here
              </a>
            </p>
            <p style="font-size:14px; color:#111; margin-bottom:0;">Regards,<br/>Capavate Team</p>
