    ...
    async (err, row) => {
      ...
      const resultsWithLogo = row.map((item) => {
        if (item.company_logo) {
          const pathname = `upload/docs/doc_${item.id}/company_profile`;
          const fullPath = `https://capavate.com/api/${pathname}/${item.company_logo}`;
  ...
  upload(req, res, function (err) {
    ...
    const formatMySQLDate = (dateStr) => {
      try {
        ...
        const match = trimmedDate.match(/^(\d{1,2})-([A-Za-z]+)-(\d{4})$/);
        ...
        if (isNaN(date.getTime())) {
          console.log("❌ Invalid date format:", trimmedDate);
          return null;
        }
        ...
        console.log("❌ Date conversion error:", error, "Input:", dateStr);
        return null;
      }
    };
