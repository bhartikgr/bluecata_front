                ...
                db.query(
                  ...
                  (err, result) => {
                    ...
                    if (invt_result.is_register === "Yes") {
                      var url = "http://blueprintcatalyst.com/investor/login";
                    } else {
                      var url = `http://blueprintcatalyst.com/investor/information/${token}`;
                    }
              ...
              db.query(
                ...
                (err, result) => {
                  ...
                  sendInvestorInviteEmail(
                    ...
                    first_name + " " + last_name,
                    companyName,
                    `http://blueprintcatalyst.com/investor/information/${token}`,
  ...
  try {
    ...
    if (sharedReportIds.length > 0) {
      await db
        .promise()
        .query(
          `UPDATE investor_updates SET is_shared = 'Yes' WHERE id IN (?)`,
          [sharedReportIds]
        );
    }
      ...
      ({ report, email, first_name, last_name, unique_code }) => {
        const url =
          report.type === "Due Diligence Document"
            ? `http://blueprintcatalyst.com/investor/company/duediligence-reportlist/${company_id}`
            : `http://blueprintcatalyst.com/investor/company/reportlist/${company_id}`;
...
exports.getInvestorReportUpdate = (req, res) => {
  ...
  db.query(query, [id, company_id], (err, results) => {
    ...
    const updatedResults = results.map((doc) => ({
      ...doc,
      downloadUrl: `http://blueprintcatalyst.com/api/${pathname}/investor_report/${doc.document_name}`,
    }));
    res.status(200).json({
      message: "",
                ...
                (err, insertResult) => {
                  ...
                  const message = `
                    ...
                    <body>
                      ...
                      <td>
                        <div style="padding:0 20px 20px 20px;">
                          <a href="http://blueprintcatalyst.com/investor/company/capital-round-list/${
                            dataa.user_id
                          }" 
...
exports.InvestorrequestToCompany = (req, res) => {
  ...
  const sqlInvestment = `
    ...
    (investor_id, roundrecord_id, company_id, shares, investment_amount, created_at)
    VALUES (?, ?, ?, ?, ?, NOW())
  `;
