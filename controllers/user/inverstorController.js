...
exports.getInvestorCompany = async (req, res) => {
  ...
  try {
    ...
    db.query(
      "SELECT company.* FROM company_investor JOIN company ON company.id = company_investor.company_id WHERE company_investor.investor_id = ? GROUP BY company.id ORDER BY MAX(company_investor.id) DESC",
      [investor_id],
      async (err, results) => {
        if (err) {
          return res
