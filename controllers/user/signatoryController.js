...
exports.signatorylogin = async (req, res) => {
  ...
  db.query(
    `SELECT cs.*, c.company_name
     FROM company_signatories cs 
     JOIN company c ON cs.company_id = c.id 
     WHERE cs.signatory_email = ? AND cs.access_status = ?`,
    [email, "active"],
    async (err, results) => {
      if (err) {
        return res
      ...
      // ✅ Iterate all rows with same email
      ...
      // ✅ Generate token for the matched row
      const token = jwt.sign(
        { email: matchedSignatory.signatory_email, role: "signatory" },
        "SECRET_KEY",
        { expiresIn: "1h" }
      );
      ...
      //     companies: [
      //       {
      //         id: matchedSignatory.company_id,
      //         name: matchedSignatory.company_name,
      //       },
      //     ],
      //   },
      // });
      db.query(
        ...
        (err2, companyRows) => {
          if (err2) {
            return res
              .status(500)
              .json({ message: "Company fetch error", error: err2 });
          }
          ...
          res.status(200).json({
            ...
            user: {
              ...
              email: matchedSignatory.signatory_email,
              role: "signatory",
              companies, // all active companies for this email
            },
          });
        }
      );
  ...
  db.query(selectQuery, [code], (err, rows) => {
    ...
    db.query(checkQuery, [email], (err2, activeRows) => {
      ...
      const updateQuery = `
        ...
        SET access_status = 'active', password = ?, viewpassword = ?
        WHERE unique_code = ?
      `;

      db.query(
        updateQuery,
        [datarow.password, datarow.viewpassword, code],
        (err3, updateResult) => {
          if (err3) {
            return res
              .status(500)
