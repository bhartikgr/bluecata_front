...
exports.getCompanyTotalShares = (req, res) => {
  ...
  const query = `
    SELECT 
      SUM(issuedshares) AS total_company_shares,currency,
      COUNT(*) AS total_rounds
    FROM roundrecord
    WHERE company_id = ? And roundStatus = ? order by id desc;
  `;
...
exports.getDilutionForecast = (req, res) => {
  ...
  db.query(query, [company_id], (err, rows) => {
    ...
    if (!rows.length) {
      return res.status(404).json({ message: "No active rounds found" });
    }
