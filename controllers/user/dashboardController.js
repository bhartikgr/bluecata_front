...
exports.getCompanyAccess = async (req, res) => {
  try {
    ...
    const [companyResults] = await db
      .promise()
      .query(
        "SELECT id AS company_id, company_name,company_email FROM company WHERE id = ? AND user_id = ?",
        [company_id, user_id],
      );
...
exports.getRoundCapTableSingleRecordForInvestor = (req, res) => {
  const { investor_id, round_id } = req.body;
  ...
  const query = `
  SELECT * FROM sharerecordround 
  WHERE investor_id = ? And roundrecord_id = ?
  ORDER BY id DESC 
  LIMIT 1
`;

  db.query(query, [investor_id, round_id], (err, result) => {
    if (err) {
      console.error("Database error:", err);
      return;
    }
...
exports.getRoundCapTableAllCompanyRecord = (req, res) => {
  const { investor_id } = req.body;
  ...
  db.query(query, [investor_id], (err, result) => {
    ...
    result.forEach((record) => {
      const round_id = record.roundrecord_id;
      const company_id = record.company_id;

      // Fetch round details with company info
      db.query(
        `SELECT r.*, c.year_registration, c.company_name
         FROM roundrecord r
         LEFT JOIN company c ON r.company_id = c.id
         WHERE r.id = ? AND r.company_id = ?`,
        [round_id, company_id],
        (err, roundResults) => {
          if (err || !roundResults || roundResults.length === 0) {
            ...
            if (processedCount === result.length) {
              return res.status(200).json({
                ...
                total_rounds: allRoundsData.length,
          ...
          if (currentRound.round_type === "Round 0") {
            ...
            allRoundsData.push({
              ...
              company_name: currentRound.company_name,
              round_id: currentRoundId,
              round_name: currentRound.nameOfRound || "Round 0",
          ...
          const calculatePercentage = (shares, totalShares) => {
            if (!totalShares || totalShares === 0) return 0;
            return (toNumber(shares) / toNumber(totalShares)) * 100;
          };
          ...
          const fetchPreFounders = () => {
            return new Promise((resolve, reject) => {
              db.query(
                `SELECT * FROM round_founders WHERE round_id=? AND company_id=? AND cap_table_type='pre' ORDER BY id ASC`,
                [currentRoundId, currentCompanyId],
                (err, results) => (err ? reject(err) : resolve(results || [])),
              );
            });
          };

          const fetchPreInvestors = () => {
            return new Promise((resolve, reject) => {
              db.query(
                `SELECT * FROM round_investors WHERE round_id=? AND company_id=? AND cap_table_type='pre' ORDER BY id ASC`,
                [currentRoundId, currentCompanyId],
          ...
          const fetchPendingInstruments = () => {
            return new Promise((resolve, reject) => {
              db.query(
                `SELECT ri.*, ri.round_name as name_of_round, 
                 ri.instrument_type as round_instrument_type,
                 ri.share_class_type as round_share_class_type
                 FROM round_investors ri
                 WHERE ri.company_id = ? 
                   ...
                   AND ri.is_pending = 1
                 ORDER BY ri.round_id ASC, ri.id ASC`,
                [currentCompanyId, currentRoundId],
                (err, results) => (err ? reject(err) : resolve(results || [])),
              );
            });
          };

          const fetchPreWarrants = () => {
            return new Promise((resolve, reject) => {
              db.query(
                `SELECT ri.*, w.expiration_date, w.id as warrant_id
                 FROM round_investors ri
          ...
          Promise.all([
            ...
            fetchPostFounders(),
            fetchPostInvestors(),
            fetchPostOptionPools(),
            fetchPendingInstruments(),
            fetchPreWarrants(),
            fetchPostWarrants(),
          ])
            .then(
              ([
                ...
                const preValidWarrants = preWarrantResults || [];
                const validPostWarrants = postWarrantResults || [];

                // Build Pre-Money Cap Table (simplified - reuse your existing logic)
                const preFounderItems = (preFounders || []).map((f) => ({
                  type: "founder",
                  founder_code: f.founder_code,
                  name: `${f.first_name || ""} ${f.last_name || ""}`.trim(),
                  email: f.email,
                  phone: f.phone,
                  shares: toNumber(f.shares),
                  shares_formatted: formatNumber(f.shares),
                  value: toNumber(f.value),
                ...
                const prePoolShares = prePool ? toNumber(prePool.shares) : 0;
                const prePoolValue = prePool ? toNumber(prePool.value) : 0;
                ...
                const preWarrantShares = preValidWarrants.reduce(
                  (s, i) => s + toNumber(i.shares),
                  0,
                );
                const preWarrantValue = preValidWarrants.reduce(
                  (s, i) => s + toNumber(i.value),
                  0,
                );
                ...
                const preTotalFounderShares = preFounderItems.reduce(
                  (s, f) => s + toNumber(f.shares),
                  0,
                );
                const preTotalFounderValue = preFounderItems.reduce(
                  (s, f) => s + toNumber(f.value),
                  0,
                );
                ...
                const postFounderItems = (postFounders || []).map((f) => ({
                  type: "founder",
                  founder_code: f.founder_code,
                  name: `${f.first_name || ""} ${f.last_name || ""}`.trim(),
                  email: f.email,
                ...
                const postPool = (postOptionPools || [])[0] || null;
                const postPoolExisting = postPool
                ...
                const postPoolNew = postPool
                  ? toNumber(postPool.new_shares)
                  : 0;
                const postPoolTotal = postPool ? toNumber(postPool.shares) : 0;
                const postPoolValue = postPool ? toNumber(postPool.value) : 0;
                ...
                const postWarrantShares = validPostWarrants.reduce(
                  (s, i) => s + toNumber(i.shares),
                  0,
                );
                const postWarrantValue = validPostWarrants.reduce(
                  (s, i) => s + toNumber(i.value),
                  0,
                );

                const postTotalFounderShares = postFounderItems.reduce(
                  (s, f) => s + toNumber(f.shares),
                  0,
                );
                const postTotalFounderValue = postFounderItems.reduce(
                  (s, f) => s + toNumber(f.value),
                  0,
                );
                ...
                // Simplified pre-money cap table object
                const preMoneyCapTable = {
                  total_shares: preTotalShares,
                  pre_money_valuation: preMoneyVal,
                  currency,
                      ...
                      ? [
                          {
                            ...
                            name: "Employee Option Pool",
                            shares: prePoolShares,
                            shares_formatted: formatNumber(prePoolShares),
                            percentage: calculatePercentage(
                              prePoolShares,
                              preTotalShares,
                            ),
                            ...
                            value: prePoolValue,
                            value_formatted: formatMoney(prePoolValue),
                            is_option_pool: true,
                          },
                        ]
                      : []),
                  ...
                  totals: {
                    total_shares: preTotalShares,
                    total_shares_formatted: formatNumber(preTotalShares),
                    total_founders: preTotalFounderShares,
                    total_option_pool: prePoolShares,
                    total_investors:
                      prePrevShares + preConvShares + preWarrantShares,
                    total_value: preTotalValue,
                    total_value_formatted: formatMoney(preTotalValue),
                    total_percentage: "100.00%",
                  },
                };

                // Simplified post-money cap table object
                const postMoneyCapTable = {
                  total_shares: postTotalShares,
                      ...
                      ? [
                          {
                            ...
                            shares_formatted: formatNumber(postPoolTotal),
                            percentage: calculatePercentage(
                              postPoolTotal,
                              postTotalShares,
                            ),
                            ...
                            value: postPoolValue,
                            value_formatted: formatMoney(postPoolValue),
                            is_option_pool: true,
                          },
                        ]
                      : []),
                  ...
                  totals: {
                    total_shares: postTotalShares,
                    total_shares_formatted: formatNumber(postTotalShares),
                    total_new_shares: postTotalNewShares,
                    total_new_shares_formatted:
                      formatNumber(postTotalNewShares),
                    ...
                    total_investors:
                      ...
                      postWarrantShares,
                    total_value: postTotalValue,
                    total_value_formatted: formatMoney(postTotalValue),
                    total_percentage: "100.00%",
                  },
                };

                // Push round data to array
                allRoundsData.push({
                  company_id: currentCompanyId,
                  company_name: currentRound.company_name,
                  round_id: currentRoundId,
            ...
            .catch((error) => {
              ...
              if (processedCount === result.length) {
                return res.status(200).json({
                  ...
                  rounds: allRoundsData,
                  total_rounds: allRoundsData.length,
                  message: "Rounds fetched with some errors",
                });
              }
            });
        },
