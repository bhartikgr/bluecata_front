...
exports.getCompanystokes = (req, res) => {
  ...
  db.query(companyQuery, [company_id], (err, companyShares) => {
    if (err) {
      console.error("Error fetching company shares:", err);
      return res.status(500).json({ success: false, message: "Server Error" });
    }

    const existingShares = companyShares.length > 0 ? parseInt(0) : 0;
    ...
    db.query(investorQuery, [company_id], (err, investorData) => {
      if (err) {
    ...
    (err, rows) => {
      ...
      db.query(
        `SELECT SUM(rr.issuedshares) as raisedToDate
         FROM company_shares_investment ci
         JOIN roundrecord rr ON ci.roundrecord_id = rr.id
         WHERE ci.roundrecord_id = ? AND ci.company_id = ?`,
        [roundId, company_id],
        (err2, investResult) => {
          if (err2) {
            return res
...
exports.getShareholder = (req, res) => {
  const { company_id, totalcompanyshare } = req.body;
