...
function handlePreferredEquityRoundSafeAndConvertibleCalculation(
  ...
  console.log(
    "🚀 Starting UNIFIED cap table calculation for:",
    round.nameOfRound,
  );
  ...
  db.query(
    ...
    (err, roundZeroData) => {
      ...
      // ✅ Get ALL previous rounds (including Common Stock, SAFE, Convertible Notes)
      db.query(
        ...
        (err, previousRounds) => {
          ...
          const safeRounds = previousRounds.filter(
            (r) => r.instrumentType === "Safe",
          );
          const convertibleNoteRounds = previousRounds.filter(
            (r) => r.instrumentType === "Convertible Note",
          );
          const commonStockRounds = previousRounds.filter(
            (r) => r.instrumentType === "Common Stock",
          );
          const preferredEquityRounds = previousRounds.filter(
            (r) => r.instrumentType === "Preferred Equity",
          );

          console.log(
            `Breakdown: ${safeRounds.length} SAFE, ${convertibleNoteRounds.length} Convertible Notes, ${commonStockRounds.length} Common Stock, ${preferredEquityRounds.length} Preferred Equity`,
          );
          ...
          try {
            if (roundZero.founder_data) {
              ...
              if (founderData.founders && Array.isArray(founderData.founders)) {
                ...
                totalExistingShares = roundZeroTotalShares;
              }
            } else {
              roundZeroTotalShares = toNumber(roundZero.issuedshares, 0);
              existingShareholders.push({
            ...
            console.log("Error parsing Round 0:", error);
            roundZeroTotalShares = toNumber(roundZero.issuedshares, 100000);
            existingShareholders.push({
              name: "Founders",
              type: "Founder",
          ...
          console.log(
            `✅ Round 0: ${roundZeroTotalShares.toLocaleString()} shares`,
          );

          // ============================================
          // ✅ STEP 2: PROCESS COMMON STOCK ROUNDS (IF ANY)
          // ============================================
          if (commonStockRounds.length > 0) {
            console.log(
              `\n🔄 Processing ${commonStockRounds.length} Common Stock round(s)`,
            );

            // For each Common Stock round, we need to simulate its effect on the cap table
            // Since we don't have the actual cap table data, we need to calculate it

            commonStockRounds.forEach((commonRound, index) => {
              try {
                const commonInvestment = toNumber(commonRound.roundsize, 0);
                const commonPreMoney = toNumber(commonRound.pre_money, 0);
                const commonTargetOptionPool = toNumber(
                  commonRound.optionPoolPercent_post,
                  0,
                );

                if (commonPreMoney > 0 && totalExistingShares > 0) {
                  // Calculate share price for this Common Stock round
                  const commonSharePrice =
                    totalExistingShares > 0
                      ? commonPreMoney / totalExistingShares
                      : 0;

                  if (commonSharePrice > 0) {
                    // Calculate shares for Common Stock investors
                    const commonShares = Math.round(
                      commonInvestment / commonSharePrice,
                    );

                    // Add Common Stock investors to cap table
                    existingShareholders.push({
                      name: `Common Stock Investor${commonStockRounds.length > 1 ? ` ${index + 1}` : ""}`,
                      type: "Common Stock Investor",
                      originalType: "Common Stock Investor",
                      source: commonRound.nameOfRound || "Common Stock Round",
                      votingRights: "voting",
                      shares: commonShares,
                      commonShares: commonShares,
                      investmentAmount: commonInvestment,
                    });

                    totalExistingShares += commonShares;

                    // Handle option pool expansion for Common Stock round
                    if (commonTargetOptionPool > 0) {
                      // Find existing option pool
                      const existingOptionPool = existingShareholders.find(
                        (sh) =>
                          sh.type === "Options Pool" ||
                          sh.originalType === "Options Pool",
                      );

                      const currentOptionPoolPercent =
                        totalExistingShares > 0
                          ? ((existingOptionPool
                              ? existingOptionPool.shares
                              : 0) /
                              totalExistingShares) *
                            100
                          : 0;

                      if (commonTargetOptionPool > currentOptionPoolPercent) {
                        // Need to expand option pool
                        const totalNonOptionShares =
                          totalExistingShares -
                          (existingOptionPool ? existingOptionPool.shares : 0);
                        const newTotalShares = Math.round(
                          totalNonOptionShares /
                            (1 - commonTargetOptionPool / 100),
                        );

                        const requiredOptionPoolShares = Math.round(
                          newTotalShares * (commonTargetOptionPool / 100),
                        );

                        const additionalShares = Math.max(
                          0,
                          requiredOptionPoolShares -
                            (existingOptionPool
                              ? existingOptionPool.shares
                              : 0),
                        );
                        ...
                        if (additionalShares > 0) {
                          if (existingOptionPool) {
                            existingOptionPool.shares += additionalShares;
                            existingOptionPool.newShares =
                              (existingOptionPool.newShares || 0) +
                              additionalShares;
                          } else {
                            existingShareholders.push({
                              name: "Employee Option Pool",
                              ...
                              originalType: "Options Pool",
                              source:
                                commonRound.nameOfRound || "Common Stock Round",
                              votingRights: "non-voting",
                          ...
                          totalExistingShares += additionalShares;
                          existingOptionPoolShares = requiredOptionPoolShares;
                          existingOptionPoolPercent = commonTargetOptionPool;
                        }
                      }
                    }

                    console.log(
                      `   Added ${commonShares.toLocaleString()} Common Stock shares @ $${commonSharePrice.toFixed(2)}`,
                    );
                  }
                }
              } catch (error) {
                console.log(
                  `Error processing Common Stock round ${index + 1}:`,
                  error,
                );
              }
            });
          ...
          // ============================================
          // ✅ STEP 3: PROCESS CONVERTIBLE ROUNDS (SAFE & NOTES)
          // ============================================
          const allConvertibles = [];
          let totalSeedInvestment = 0;
          let convertibleOptionPoolPercent = 0;
          ...
          // First, check if there are convertible rounds
          if (safeRounds.length > 0 || convertibleNoteRounds.length > 0) {
            const convertibleRounds = [...safeRounds, ...convertibleNoteRounds];
            ...
            // Calculate employee shares from convertible rounds
            if (convertibleOptionPoolPercent > 0 && totalExistingShares > 0) {
              // Find current option pool
              const currentOptionPool = existingShareholders.find(
                (sh) =>
                  sh.type === "Options Pool" ||
                  sh.originalType === "Options Pool",
              );
              ...
              if (currentOptionPool) {
                // Already have option pool from Common Stock rounds
                existingOptionPoolShares = currentOptionPool.shares;
                existingOptionPoolPercent =
                  (existingOptionPoolShares / totalExistingShares) * 100;

                if (convertibleOptionPoolPercent > existingOptionPoolPercent) {
                  // Need to expand option pool to meet convertible round requirement
                  const requiredShares = Math.round(
                    totalExistingShares /
                      (1 - convertibleOptionPoolPercent / 100) -
                      totalExistingShares,
                  );

                  const additionalShares = Math.max(
                    0,
                    requiredShares - existingOptionPoolShares,
                  );

                  if (additionalShares > 0) {
                    currentOptionPool.shares += additionalShares;
                    currentOptionPool.newShares =
                      (currentOptionPool.newShares || 0) + additionalShares;
                    totalExistingShares += additionalShares;
                    existingOptionPoolShares = currentOptionPool.shares;
                    existingOptionPoolPercent = convertibleOptionPoolPercent;
                  }
                ...
                // No existing option pool, create one
                employeeSharesSeedRound = Math.round(
                  totalExistingShares /
                    (1 - convertibleOptionPoolPercent / 100) -
                    totalExistingShares,
          ...
          // ============================================
          // ✅ STEP 4: CALCULATE SHARE PRICE FOR PREFERRED EQUITY
          // ============================================
          const seriesAInvestment = toNumber(round.roundsize, 0);
          const preMoneyValuation = toNumber(round.pre_money, 0);
          const targetOptionPoolPercent = toNumber(
          ...
          // Calculate share price based on current cap table
          const sharePrice =
            totalExistingShares > 0
              ? preMoneyValuation / totalExistingShares
              : 0;
          ...
          console.log(
            `   Series A Investment: ${seriesAInvestment.toLocaleString()}`,
          );
          console.log(`   Target Option Pool: ${targetOptionPoolPercent}%`);
          console.log(
            `   Existing Option Pool: ${existingOptionPoolPercent.toFixed(1)}% (${existingOptionPoolShares.toLocaleString()} shares)`,
          );

          if (sharePrice <= 0) {
            return res.status(400).json({
              success: false,
              message: "Invalid share price calculation",
            });
          }

          // ============================================
          // ✅ STEP 5: CONVERT CONVERTIBLES AT NEW SHARE PRICE
          // ============================================
          const convertedInvestors = [];
          let totalSeedConversionShares = 0;
          let totalSeedConversionValue = 0;

          if (allConvertibles.length > 0) {
            console.log(
              `\n🔄 Converting ${allConvertibles.length} convertible instrument(s):`,
            );
            ...
            allConvertibles.forEach((convertible, index) => {
              ...
              console.log(
                `   ${convertible.instrumentType}: ${seedConversionShares.toLocaleString()} shares @ $${optimalPrice.toFixed(2)}`,
              );
            });
          }

          // ============================================
          // ✅ STEP 6: CALCULATE PREFERRED EQUITY SHARES
          // ============================================
          const seriesAShares = Math.round(seriesAInvestment / sharePrice);
          const seriesAValue = seriesAShares * sharePrice;

          // ============================================
          // ✅ STEP 7: OPTION POOL CALCULATION
          // ============================================
          let newOptionShares = 0;
          let totalPostShares = 0;
          ...
          if (targetOptionPoolPercent > 0) {
            // Total shares excluding existing option pool
            ...
            const totalRequiredOptionPoolShares = Math.round(
              totalPostShares * (targetOptionPoolPercent / 100),
            );
            ...
            totalPostShares =
              totalExistingShares + totalSeedConversionShares + seriesAShares;
            finalPostMoneyValuation = totalPostShares * sharePrice;
          }
          ...
          console.log(
            `   Post-Money Valuation: ${finalPostMoneyValuation.toLocaleString()}`,
          );
          console.log(
          ...
          // ============================================
          // ✅ STEP 8: BUILD CAP TABLES (SAME AS BEFORE)
          // ============================================

          // Build pre-investment cap table
          const preSeriesAShareholders = existingShareholders.map((sh) => {
            const ownership =
              totalExistingShares > 0
                ? (sh.shares / totalExistingShares) * 100
                : 0;
            const value = (ownership / 100) * preMoneyValuation;
          ...
          // Build post-investment cap table
          const postSeriesAShareholders = [];
          ...
          existingShareholders.forEach((sh) => {
            ...
            const ownership =
              totalPostShares > 0 ? (sh.shares / totalPostShares) * 100 : 0;
            const value = (ownership / 100) * finalPostMoneyValuation;
          ...
          convertedInvestors.forEach((conversion, index) => {
            ...
            postSeriesAShareholders.push({
              ...
              source:
                "Converted in " +
                (round.nameOfRound || "Preferred Equity Round"),
              votingRights: "voting",
          ...
          if (seriesAShares > 0) {
            ...
            postSeriesAShareholders.push({
              ...
              originalType: "Preferred Investor",
              source: round.nameOfRound || "Preferred Equity Round",
              votingRights: "voting",
              shares: seriesAShares,
              commonShares: 0,
              newShares: seriesAShares,
          ...
          // Add Option Pool
          const totalRequiredOptionPoolShares =
            existingOptionPoolShares + newOptionShares;
          if (totalRequiredOptionPoolShares > 0) {
            const ownership =
              totalPostShares > 0
                ? (totalRequiredOptionPoolShares / totalPostShares) * 100
                : 0;
            const value = (ownership / 100) * finalPostMoneyValuation;
          ...
          let totalAllValue = 0;
          let totalInvestment = seriesAInvestment + totalSeedInvestment;
          ...
          postSeriesAShareholders.push({
            ...
            commonShares: totalExistingShares - existingOptionPoolShares,
            newShares:
              totalAllShares - (totalExistingShares - existingOptionPoolShares),
          ...
          const responseData = {
            success: true,
            roundType: round.nameOfRound || "Preferred Equity Round",
            round_type: round.nameOfRound || "Preferred Equity Round",
            shareClassType: round.shareClassType,
            ...
            previousRoundsInfo: {
              commonStockRounds: commonStockRounds.length,
              safeRounds: safeRounds.length,
              convertibleNoteRounds: convertibleNoteRounds.length,
              totalPreviousRounds: previousRounds.length,
            },
            ...
            calculations: {
              ...
              newOptionShares: newOptionShares,
              totalSharesPreSeriesA: totalExistingShares,
              totalSharesPostSeriesA: totalPostShares,
              preMoneyValuation: preMoneyValuation,
            ...
            preSeedCapTable: {
              ...
              totalShares: totalExistingShares,
              totalValue: preMoneyValuation,
              message: `Before ${round.nameOfRound || "Preferred Equity"} investment`,
            },
            ...
            postSeedCapTable: {
              ...
              message: `After ${round.nameOfRound || "Preferred Equity"} investment of ${seriesAInvestment.toLocaleString()} ${round.currency || "USD"}`,
            },
          };

          console.log("\n✅ CAP TABLE CALCULATION COMPLETE");
          console.log(`   Total Shares: ${totalPostShares.toLocaleString()}`);
          console.log(
            `   Post-Money: ${finalPostMoneyValuation.toLocaleString()}`,
          );

          return res.status(200).json({
            success: true,
            message: "Cap table calculated successfully",
