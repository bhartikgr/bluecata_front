...
exports.getPreviousRoundForAutoFill = (req, res) => {
  const { company_id, current_round_id, current_instrument_type } = req.body;
  ...
  db.query(getPreviousRoundQuery, params, (err, results) => {
    ...
    // ✅ CRITICAL FIX: GET CUMULATIVE OPTION POOL FROM ALL PREVIOUS ROUNDS
    ...
    db.query(getCumulativePoolQuery, poolParams, (poolErr, poolResults) => {
      ...
      console.log("\n" + "=".repeat(80));
      console.log("📊 CUMULATIVE OPTION POOL CALCULATION");
      console.log("=".repeat(80));
      console.log(`Total Rounds: ${poolResults[0]?.round_count || 0}`);
      console.log(
        `Cumulative Option Pool Shares: ${cumulativeOptionPoolShares.toLocaleString()}`,
      );
      console.log(
        `Total Shares After Previous: ${totalShares.toLocaleString()}`,
      );
      ...
      if (
        ...
        // ✅ FORMULA: Post-money pool % = (Cumulative Option Pool / Total Shares) × 100
        previousPostMoneyPool =
          (cumulativeOptionPoolShares / totalShares) * 100;
        console.log(
          `📊 CALCULATED Post-money pool: ${previousPostMoneyPool.toFixed(2)}%`,
        );
      } else if (dbPostMoneyPool === 0 && dbPreMoneyPool > 0) {
        previousPostMoneyPool = dbPreMoneyPool;
        console.log(
          `📊 USING Pre-money pool: ${previousPostMoneyPool.toFixed(2)}%`,
        );
      } else {
        console.log(
          `📊 USING Database post-money pool: ${previousPostMoneyPool.toFixed(2)}%`,
        );
      }
      ...
      if (totalShares > 0 && cumulativeOptionPoolShares > 0) {
        employeeOwnershipPercent =
          (cumulativeOptionPoolShares / totalShares) * 100;
        console.log(
          `✅ Employee ownership: ${employeeOwnershipPercent.toFixed(2)}%`,
        );
      } else {
        console.log(`⚠️ Employee ownership: 0% (no option pool or no shares)`);
      }

      // ✅ DETERMINE ROUND TYPE
      const isPricedRound =
        current_instrument_type === "Common Stock" ||
        current_instrument_type === "Preferred Equity";

      const isUnpricedRound =
        current_instrument_type === "Safe" ||
        current_instrument_type === "Convertible Note";

      const canAutofill = isPricedRound && previousPostMoneyPool > 0;

      console.log("\n📋 AUTOFILL DECISION:");
      console.log(`Current Instrument: ${current_instrument_type}`);
      console.log(`Is Priced Round: ${isPricedRound}`);
      console.log(`Can Autofill: ${canAutofill}`);
      console.log("=".repeat(80) + "\n");
      ...
      const responseData = {
        ...
        existingOptionPoolPercent: canAutofill
        ...
        note: canAutofill
          ? `Previous round had ${previousPostMoneyPool.toFixed(2)}% post-money pool. Autofilling ${previousPostMoneyPool.toFixed(2)}% for pre-money pool.`
          : isUnpricedRound
            ? `Unpriced round (${current_instrument_type}) - no autofill.`
            : `Previous round had 0% post-money pool.`,
      };
