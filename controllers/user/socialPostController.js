    ...
    const enriched = posts.map((p) => {
      ...
      if (p.author_raw_image) {
        if (p.author_type === "company") {
          authorImage = `${process.env.API_BASE_URL || "https://capavate.com/api"}/upload/docs/doc_${p.author_id}/company_profile/${p.author_raw_image}`;
        } else {
          authorImage = `${process.env.API_BASE_URL || "https://capavate.com/api"}/upload/investor/inv_${p.author_id}/${p.author_raw_image}`;
        }
      }
