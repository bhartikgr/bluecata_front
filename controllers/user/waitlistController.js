...
exports.interestInvestor = async (req, res) => {
  ...
  if (!company_id) {
    return res
      .status(400)
      .json({ status: "2", message: "Company ID is required" });
  }

  try {
    const query = `
      SELECT 
        cii.id as interest_id,
        cii.created_at as interested_at,
        ...
        ii.country,
        ii.type_of_investor,
        -- ✅ Check karo ki investor already company mein join hai ya nahi
        CASE WHEN ci.id IS NOT NULL THEN 'Yes' ELSE 'No' END as already_joined,
        ci.joinstatus,
        ci.crm_status
      FROM company_investor_interest cii
      INNER JOIN investor_information ii 
        ON ii.id = cii.investor_id
      -- ✅ company_investor se extra info (optional)
      LEFT JOIN company_investor ci 
        ON ci.investor_id = cii.investor_id 
        AND ci.company_id = cii.company_id
      WHERE cii.company_id = ?
    ...
    db.query(query, [company_id], (err, results) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ status: "2", message: "Database error", error: err.message });
      }
    ...
    return res
      .status(500)
      .json({ status: "2", message: "Server error", error: err.message });
  }
};
