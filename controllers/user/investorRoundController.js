...
exports.getcompanyDetailsForInvestor = (req, res) => {
  ...
  const query = `
    SELECT 
      ...
      cli.office_address,
      cli.mailing_address,
      -- ✅ company_investor fields
      ci.joinstatus,
      ci.archived_status,
      ci.crm_status
    FROM company c
    LEFT JOIN company_legal_information cli ON cli.company_id = c.id
    JOIN company_investor ci 
      ON ci.company_id = c.id 
      AND ci.investor_id = ?
    WHERE c.id = ?
  `;

  db.query(query, [investor_id, company_id], (err, row) => {
    if (err) {
      return res.status(500).json({
        status: 0,
        message: "Database query error",
...
exports.archiveInvitation = (req, res) => {
  const {
    company_investor_id_id,
    investor_id,
    company_name,
    company_email,
    company_id,
  ...
  if (!company_investor_id_id) {
    return res.status(400).json({
      status: 0,
      message: "company_investor_id_id is required",
    });
  }

  const query = `
    UPDATE company_investor 
    SET archived_status = 'Yes', updated_at = NOW()
    WHERE id = ? AND investor_id = ?
  `;

  db.query(query, [company_investor_id_id, investor_id], (err, result) => {
    if (err) {
      return res.status(500).json({
        status: 0,
        message: "Database query error",
    ...
    if (result.affectedRows === 0) {
      return res.status(404).json({
        status: 0,
        message: "No record found to archive",
      });
    }
    insertAccessLog({
      investor_id,
...
exports.unarchiveInvitation = (req, res) => {
  const {
    company_investor_id_id,
    investor_id,
    company_name,
  ...
  const query = `
    UPDATE company_investor 
    SET archived_status = 'No', updated_at = NOW()
    WHERE id = ? AND investor_id = ?
  `;
