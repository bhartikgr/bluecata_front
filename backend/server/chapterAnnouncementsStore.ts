/**
 * server/chapterAnnouncementsStore.ts — v25.34 Collective Mega-Wave (verified DB-direct)
 *
 * ===========================================================================
 * v25.34 CHANGE BLOCK
 * ---------------------------------------------------------------------------
 ...
 *   - No in-memory Map/array source of truth (the `conditions` array is just a
 *     drizzle WHERE-builder).
 *   - All reads (list / detail / findAnnouncementById) query SQLite directly
 *     via getDb().select(...).all(). On read failure they return an empty,
 *     `degraded: true` payload — they NEVER fall back to stale memory.
 *   - All writes (POST/PATCH/DELETE/pin) run inside db.transaction() and are
 *     FAIL-CLOSED: a DB failure is logged via log.error and the route returns
 *     HTTP 500 (no silent cache mutation).
