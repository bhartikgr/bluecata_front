...
function persistChannel(ch: TransactionPrepChannel): void {
  try {
    ...
    // v25.35 Phase 4 #22 — fail-closed: persistChannel is the single durable
    // write path for M&A transaction-prep channels. Previously it swallowed,
    // so a failed write left the channel (and its hash-chain advance) in memory
    // only — lost on the next restart. Throw so callers can roll back the
    // in-memory cache + chain mutation and surface a 500.
    log.error("[transactionPrepStore.persistChannel] write-through failed:", (err as Error).message);
    throw err;
  }
}
...
export function createChannel(input: { companyId: string; founderUserId: string; initialMemberUserIds?: string[] }): TransactionPrepChannel {
  return withTrace("comms.transaction_prep.create", "1.0.0", "US", () => {
    ...
    // v25.35 Phase 4 #22 — persist FIRST; only mutate the cache + advance the
    // hash chain after the durable write commits. If persistChannel throws,
    // nothing was written to memory and the throw propagates to the route (500).
    persistChannel(ch);
    channels.set(id, ch);
...
export function archiveChannel(channelId: string, reason: "not_pursuing" | "transaction_closed", actorUserId: string): TransactionPrepChannel | undefined {
  ...
  return withTrace("comms.transaction_prep.archive", "1.0.0", "US", () => {
    // v25.35 Phase 4 #22 — fail-closed: stage the archive on a clone, persist
    // first, and only commit the mutation to the live in-memory object + chain
    // after the durable write succeeds. On failure the live channel is left
    // untouched and the throw surfaces as a 500.
    const archivedAt = new Date().toISOString();
    const staged: TransactionPrepChannel = { ...ch, archivedAt, archiveReason: reason };
    persistChannel(staged);
    ch.archivedAt = archivedAt;
...
export function addMember(channelId: string, userId: string): TransactionPrepChannel | undefined {
  ...
  // v25.35 Phase 4 #22 — fail-closed: persist the new member list first; only
  // mutate the live channel + advance the chain after the durable write
  // commits. On failure the member is NOT added in memory and the throw
  // surfaces as a 500.
  const staged: TransactionPrepChannel = { ...ch, memberUserIds: [...ch.memberUserIds, userId] };
  persistChannel(staged);
...
export function registerTransactionPrepRoutes(app: Express): void {
  ...
  app.post("/api/founder/comms/transaction-prep", (req: Request, res: Response) => {
    ...
    // v25.35 Phase 4 #22 — createChannel now fails-closed; translate a durable
    // write failure into a 500 instead of a memory-only 201.
    let ch;
    try {
      ch = createChannel(parsed.data);
    } catch (err) {
      log.error("[transactionPrep.createRoute] persist failed:", (err as Error).message);
      return res.status(500).json({ ok: false, error: "TXPREP_PERSIST_FAILED", message: "Could not create the transaction-prep channel. Please retry." });
    }
  ...
  app.post("/api/founder/comms/transaction-prep/:id/archive", (req: Request, res: Response) => {
    ...
    // v25.35 Phase 4 #22 — archiveChannel now fails-closed; translate a durable
    // write failure into a 500 instead of acknowledging a memory-only archive.
    let ch;
    try {
      ch = archiveChannel(req.params.id, reason, actor);
    } catch (err) {
      log.error("[transactionPrep.archiveRoute] persist failed:", (err as Error).message);
      return res.status(500).json({ ok: false, error: "TXPREP_PERSIST_FAILED", message: "Could not archive the transaction-prep channel. Please retry." });
    }
