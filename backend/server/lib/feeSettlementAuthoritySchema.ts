/**
 * server/lib/feeSettlementAuthoritySchema.ts — WAVE 3E.
 *
 * GENERATED FILE. DO NOT HAND-EDIT.
 * Source: migrations/0151_wave3e_fee_settlement_authorization.sql
 * Generator: _w3e/gen_authority_schema.py
 *
 * WHY THIS EXISTS
 * ---------------
 * The durable settlement-authority tables must exist on the `:memory:` test
 * path and on any database that has not yet run the migration runner. The
 * project's normal third home for that bootstrap is `applyInlineMigrations()`
 * in `server/db/connection.ts`, which is SACRED and must not be edited by this
 * wave. The bootstrap therefore lives with the module that owns the table, and
 * it executes the CANONICAL MIGRATION TEXT VERBATIM rather than a hand-copied
 * paraphrase that could drift.
 *
 * The text below is a byte-for-byte copy of the migration.
 * `server/__tests__/wave3e_settlement_authority_db.test.ts` asserts that
 * equality against the file on disk, so drift fails the suite loudly.
 *
 * NOTE: this SQL creates EMPTY tables and seeds NOTHING. Bootstrapping can
 * never manufacture an authorization; a fresh database settles nothing.
 */
export const FEE_SETTLEMENT_AUTHORITY_SQL = `-- migrations/0151_wave3e_fee_settlement_authorization.sql
