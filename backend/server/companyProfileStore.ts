...
export function updateCompanyProfile(
  ...
  try {
    ...
    /* v25.33 final — FAIL CLOSED on DB write failure.
     *
     * The prior behavior logged the error and updated the in-memory
     * profileMap anyway, returning a success-shaped result to the caller.
     * That violated Ozan's "nothing in memory / all DB-driven" rule — a
     * user could see a "saved" toast for a write that never persisted, and
     * the next process restart would wipe the change. The old comment
     * explicitly tagged this as the "production fix path: surface this
     * back to the route" — this is that fix.
     *
     * The route handler in routes.ts catches and returns 500. The user
     * sees an error toast and can retry. The in-memory cache is NOT
     * updated, so subsequent reads see the durable (pre-patch) state —
     * matching what's actually in the DB. */
    log.error(
