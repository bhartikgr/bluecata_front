...
export function registerCollectiveNetworkRoutes(app: Express): void {
  ...
  app.get("/api/investor/companies/:id/co-members", async (req: Request, res: Response) => {
    try {
      ...
      // v25.40 FIX-11: use the structured logger instead of console.warn,
      // matching the v25.35 logger pattern used across the collective stores.
      log.warn("[co-members]", (err as Error).message);
      return res.json([]);
    }
  });
}
