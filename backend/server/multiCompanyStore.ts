...
export type FounderCompanyMembership = {
  ...
  /* v25.33 P0a -- region / tagline / description from the Settings -> Company
   * form. The client always sent these in the PATCH body but the server
   * discarded them (they were never passed to updateCompanyDetails). They are
   * now persisted to the durable `company_settings_overview` side table and
   * overlaid here on read so the Settings form round-trips them. Additive over
   * Avi's writes; reads DB-direct. */
  region?: string;
  tagline?: string;
  description?: string;
};
...
/* v25.33 P0a -- durable region / tagline / description for Settings -> Company.
 * Mirrors the company_default_currency side-table pattern (v24.2 Bug 6). Only
 * the fields explicitly present in `patch` are written; absent fields are left
 * untouched (COALESCE), so a partial PATCH never blanks a previously-saved
 * value. Throws on DB failure so the caller can surface a 500 and avoid silent
 * cache/durable divergence. Additive over Avi's writes; reads DB-direct. */
export function writeCompanyOverview(
  companyId: string,
  tenantId: string,
  patch: { region?: string; tagline?: string; description?: string },
): void {
  const hasAny =
    typeof patch.region === "string" ||
    typeof patch.tagline === "string" ||
    typeof patch.description === "string";
  if (!hasAny) return;
  const db = rawDb();
  db.prepare(
    `INSERT INTO company_settings_overview (company_id, tenant_id, region, tagline, description, updated_at, deleted_at)
     VALUES (?, ?, ?, ?, ?, ?, NULL)
     ON CONFLICT(company_id) DO UPDATE SET
       tenant_id   = excluded.tenant_id,
       region      = COALESCE(excluded.region, company_settings_overview.region),
       tagline     = COALESCE(excluded.tagline, company_settings_overview.tagline),
       description = COALESCE(excluded.description, company_settings_overview.description),
       updated_at  = excluded.updated_at,
       deleted_at  = NULL`,
  ).run(
    companyId,
    ...
    typeof patch.tagline === "string" ? patch.tagline : null,
    typeof patch.description === "string" ? patch.description : null,
    new Date().toISOString(),
  );
}

/** Read a company's durable Settings overview (region/tagline/description). */
export function readCompanyOverview(
  companyId: string,
): { region?: string; tagline?: string; description?: string } {
  try {
    const db = rawDb();
    const row = db.prepare(
      `SELECT region, tagline, description FROM company_settings_overview WHERE company_id = ? AND deleted_at IS NULL`,
    ).get(companyId) as { region?: string | null; tagline?: string | null; description?: string | null } | undefined;
    if (!row) return {};
    const out: { region?: string; tagline?: string; description?: string } = {};
    if (row.region != null) out.region = row.region;
...
function dbRowToMembership(coRow: any, memRow: any): FounderCompanyMembership {
  ...
  return {
    ...
    // v25.33 P0a — overlay durable region / tagline / description so the
    // Settings → Company form round-trips them after save + reload.
    ...readCompanyOverview(coRow.id),
  };
}
...
export function updateCompanyDetails(
  companyId: string,
  patch: Partial<Pick<FounderCompanyMembership, "companyName" | "legalName" | "sector" | "stage" | "hq" | "role" | "defaultCurrency" | "region" | "tagline" | "description">>,
  ...
  for (const [userId, companies] of USER_COMPANIES.entries()) {
    ...
    const next: FounderCompanyMembership = {
      ...
      // v25.33 P0a -- region / tagline / description were previously discarded
      // at save time. Mirror them into the cache and persist below to the
      // company_settings_overview side table.
      ...(typeof patch.region      === "string" ? { region: patch.region } : {}),
      ...(typeof patch.tagline     === "string" ? { tagline: patch.tagline } : {}),
    ...
    try {
      ...
      // v25.33 P0a -- persist region / tagline / description to their durable
      // side table so they survive restart and round-trip through
      // GET /api/founder/active-company. Only writes fields present in the
      // patch (COALESCE in the upsert). Inside the same try so a DB failure is
      // fatal (no silent cache/durable divergence) per v24.2 Bug 6 semantics.
      writeCompanyOverview(companyId, tenantId, {
        region: typeof patch.region === "string" ? patch.region : undefined,
