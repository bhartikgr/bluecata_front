...
/**
 * Look up a single offer by id within the caller's tenant scope.
 *
 * Returns null if not found OR if cross-tenant (`withTenant` filters it
 * out). The route uses this to short-circuit a 404.
 */
export function findOfferById(offerId: string, tenantId: string): OfferRow | null {
  try {
    const db: any = getDb();
    const rows = db
      .select()
    ...
    if (!/no such table/i.test(msg)) {
      log.warn("[collectiveOffersStore.findOfferById] DB read failed:", msg);
    }
    return null;
  }
}
