...
function buildSnapshots(rounds: ApiRound[], securities: ApiSecurity[]): Snapshot[] {
 // Filter to NovaPay only, sort by closeDate ascending.
 const ours = rounds
 .filter((r) => (r as any).companyId === COMPANY_ID || r.company === "NovaPay AI")
...
export default function CapitalizationJourney() {
 const rounds = useQuery<ApiRound[]>({ queryKey: ["/api/rounds"] });
 const securities = useQuery<ApiSecurity[]>({ queryKey: [`/api/companies/${COMPANY_ID}/securities`] });
 ...
 return buildSnapshots(novapayRounds, securities.data);
 }, [novapayRounds, securities.data]);
 ...
 The historical view of NovaPay AI's capitalization — every round, every valuation step-up, every dilution event, plotted from foundation to today.
 </p>
 </div>
 <Tooltip>
