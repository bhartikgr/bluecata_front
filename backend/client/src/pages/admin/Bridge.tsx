...
export default function AdminBridge() {
  ...
  const emit = async (eventType: string) => {
    await apiRequest("POST", "/api/admin/bridge/emit", { eventType, aggregateId: "co_novapay", aggregateKind: "company", payload: { manual: true } });
    qc.invalidateQueries({ queryKey: ["/api/admin/bridge/outbox"] });
    qc.invalidateQueries({ queryKey: ["/api/admin/bridge/verify-chain"] });
  };
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-2 text-xs">
            {(out.data?.eventTypes ?? []).map(t => (
              <button
                ...
                className="flex items-center gap-2 px-3 py-2 border border-border rounded text-left hover:bg-accent/30"
              >
