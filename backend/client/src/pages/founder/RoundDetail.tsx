...
import { fmtUSD, fmtPct, fmtDate, timeAgo, fmtNum } from "@/lib/format";
...
export default function RoundDetail() {
 ...
 <CommitPipeline roundId={r.id} companyId={(r as any).companyId ?? "co_novapay"} />
...
function ProjectionPanel({ round }: { round: Round }) {
 const { toast } = useToast();
 const securities = useQuery<ApiSecurity[]>({ queryKey: ["/api/companies/co_novapay/securities"] });
 ...
 New investor allocation: <strong className="text-foreground">{fmtUSD(round.targetAmount)}</strong> at <strong className="text-foreground">${round.pricePerShare.toFixed(4)}</strong>/share — {pre.formulaIdsUsed.length} → {post.formulaIdsUsed.length} formulas applied.
 </div>
 <div className="mt-3">
