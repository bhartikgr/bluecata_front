/**
 * ISO-3166 country list (250 entries) — production-shape.
 *
 * Each entry maps `code` (ISO-3166 alpha-2, the value Postgres stores as
 * `country_code`) to its display `name`. Names match the live Capavate site
 * exactly — see capavate_investor_deep_audit.md §3 for the source list.
 *
 * The cap-table engine `Region` is derived from `code` via `regionForCountry`.
 * See `region.ts` in this directory.
 */

export interface Country {
  code: string;   // ISO-3166 alpha-2 (production column: country_code)
  name: string;   // Display label
  dialCode?: string; // E.164 country dial code (production column: phone_country_code)
}
...
export const COUNTRIES: readonly Country[] = [
  ...
  { code: "MK", name: "Macedonia (North)", dialCode: "+389" },
  ...
  { code: "TL", name: "Timor-Leste (East Timor)", dialCode: "+670" },
