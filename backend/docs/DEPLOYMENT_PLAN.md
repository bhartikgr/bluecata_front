...
| `COLLECTIVE_WEBHOOK_URL` | no — leave BLANK | `https://capavate.com/api/bridge/inbound` | Outbound mutation fan-out target. **`collective.capavate.com` does not resolve**; the Collective is a path on `capavate.com`. Leave this blank while `BRIDGE_ENABLED=0` — `server/lib/bridgeRuntime.ts` validates it at module load, so a set-but-unreachable value aborts boot. If enabling, `COLLECTIVE_WEBHOOK_SECRET` and `BRIDGE_INBOUND_HMAC_SECRET` must be the SAME value (it is a self-loop). |
