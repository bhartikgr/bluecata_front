...
export const rounds = sqliteTable("rounds", {
  ...
  // the correct denominator for PPS in a priced round (existing common +
  // preferred as-converted + granted options + option pool reserved + SAFE/note
  // conversions). Nullable; only required for non-foundation priced rounds.
  // Populated by migration 0127 via ALTER TABLE ADD COLUMN.
  fdPreMoneyShares: integer("fd_pre_money_shares"),
  pricePerShare: real("price_per_share"),
...
export const INSTRUMENTS = [
  {
    ...
    // PPS automatically (PPS = pre-money ÷ FD pre-money shares, grossed up
    // for any option-pool top-up on this round). The Foundation flow inside
    // RoundNew.tsx hides those two inputs (foundation has no prior valuation
    // and creates the first shares) and falls back to a manual PPS, so this
    // field-set addition is safe for the foundation path. The server also
    // enforces the foundation-vs-post-formation rule in POST /api/rounds.
    fields: ["preMoney", "fdPreMoneyShares", "sharesAuthorized", "pricePerShare"],
  },
  {
    value: "preferred",
    ...
    // Wave C v26.5.0 (Shadie Finding 1a) — fdPreMoneyShares added so priced
    // preferred rounds use the mathematically correct FD pre-money denominator
    // for PPS, not the ambiguous sharesAuthorized (which is new-shares-issued).
    // Existing preferred rounds without fdPreMoneyShares are handled at the
    // client validation layer (reqPos error surfaces the missing input).
    fields: ["preMoney", "fdPreMoneyShares", "targetAmount", "pricePerShare", "sharesAuthorized", "liqPrefMultiple", "participating", "capParticipation", "antiDilution"],
  },
  {
    value: "safe_post",
...
export const partnerDealPipeline = sqliteTable("partner_deal_pipeline", {
  ...
  dealSizeUsd: real("deal_size_usd"),
 ...
 *    verified across all 2714 pre-existing lines), so every FK below is expressed as a
...
export const mfcStages = sqliteTable("mfc_stages", {
  id: text("id").primaryKey(),
  partnerId: text("partner_id").notNull(),
...
export const mfcStageTransitions = sqliteTable("mfc_stage_transitions", {
  id: text("id").primaryKey(),
  partnerId: text("partner_id").notNull(),
  stageMachineType: text("stage_machine_type").notNull(),
...
export const authorityArtifacts = sqliteTable("authority_artifacts", {
  id: text("id").primaryKey(),
  partnerId: text("partner_id").notNull(),
...
export const partnerCrmContactClientScope = sqliteTable("partner_crm_contact_client_scope", {
  id: text("id").primaryKey(),
...
export const partnerCompanyRelationship = sqliteTable("partner_company_relationship", {
  id: text("id").primaryKey(),
  partnerId: text("partner_id").notNull(),
  companyId: text("company_id").notNull(),
  createdAt: text("created_at").notNull(),
...
export const pcrSurfacePresence = sqliteTable("pcr_surface_presence", {
  id: text("id").primaryKey(),
...
export const mfcClassificationRequests = sqliteTable("mfc_classification_requests", {
  id: text("id").primaryKey(),
  partnerId: text("partner_id").notNull(),
...
export const c2BackfillSkipLog = sqliteTable("c2_backfill_skip_log", {
  id: text("id").primaryKey(),
...
export const c2PipelineBackfillLock = sqliteTable("_c2_pipeline_backfill_lock", {
  id: text("id").primaryKey(),
  startedAt: text("started_at").notNull(),
