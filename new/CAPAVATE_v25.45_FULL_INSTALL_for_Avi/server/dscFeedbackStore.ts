...
export function ingestDscScores(input: {
  ...
  return withTrace("comms.dsc.feedback_relay", "1.0.0", "US", () => {
    ...
    // v25.35 — FAIL-CLOSED (BLOCKER #8): previously the DB write was swallowed
    // and BOTH the in-memory mirror AND the tamper-evident hash chain advanced
    // regardless — implying a durable, reviewed DSC score the DB cannot
    // recover. We now throw on DB failure, and the hash-chain append + cache
    // set move INSIDE the try AFTER the successful commit so the chain only
    // ever advances on durable records.
    try {
      const db: any = getDb();
      ...
      // v25.35 — only after a durable commit: advance the hash chain and the
      // in-memory mirror. The chain must reflect persisted records only.
      items.set(id, f);
      dscFeedbackChain.append({ id, companyId: f.companyId, tier: f.tier, ts: f.receivedAt });
    } catch (err) {
      // v25.35 — fail-closed: do NOT advance the chain or cache; surface to the
      // route as 500.
      log.error(
        "[dscFeedbackStore.ingestDscScores] DB write failed:",
