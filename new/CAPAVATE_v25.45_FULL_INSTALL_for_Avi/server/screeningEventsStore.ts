/**
 * server/screeningEventsStore.ts — v25.34 Collective Mega-Wave (verified DB-direct)
 *
 * ===========================================================================
 * v25.34 CHANGE BLOCK
 * ---------------------------------------------------------------------------
 * AUDIT RESULT: this store is ALREADY fully DB-direct and fail-closed; NO
 * behavioral change was required for v25.34:
 *   - No in-memory Map/array source of truth. Every read
 *     (getChapterMembership / findEventByIdAnyTenant / listAttendeesForEvent /
 *     list / detail endpoints) queries SQLite directly via
 *     getDb().select(...).all(). On read failure they return null / [] — there
 *     is no stale-memory fallback because there is no memory.
 *   - Every write (create / cancel / check-in / RSVP) runs inside
 *     db.transaction() and is FAIL-CLOSED: a DB failure is logged via
 *     log.error and the route returns HTTP 500. The capacity/race guards even
 *     `throw` from inside the tx to force a rollback (see event_missing /
 *     race_cancelled).
 * Per the v25.34 conservatism rule, working drizzle code is preserved and only
 * this documentation header is added. Satisfies Ozan's rule #1.
 * ===========================================================================
 *
