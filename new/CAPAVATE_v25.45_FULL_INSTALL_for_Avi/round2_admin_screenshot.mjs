...
// Give React + TanStack Query time to fetch /api/admin/dashboard/kpis and render.
await page.waitForTimeout(4000);

const bodyText = await page.locator('body').innerText({ timeout: 8000 }).catch(() => '');

// 3) Assertions — the page must render WITHOUT the ErrorBoundary fallback.
const hasErrorBoundary = /Something went wrong/i.test(bodyText);
// Positive signals the real dashboard rendered:
...
// The em-dash placeholder should appear where successRate/deliveryRate are null.
...
await page.screenshot({ path: `${outDir}/round2_admin_dashboard.png`, fullPage: true });

const pass = !hasErrorBoundary && hasHeader && hasHealthTile > 0 && hasKpiCard > 0;
log.push({ VERDICT: pass ? 'PASS — renders without ErrorBoundary' : 'FAIL', errors });
fs.writeFileSync(`${outDir}/round2_admin_capture_log.json`, JSON.stringify(log, null, 2));
