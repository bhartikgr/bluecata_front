 ...
 *
 * The Sprint 16 Tier 1/2/3 comms stores were RAM-only, which meant every
 * server restart wiped co-investor groups, IOI pulses, endorsements, Q&A
 * threads, and diligence volunteer records. For a launch-ready platform
 * this is unacceptable.
 *
 * Strategy: each in-memory array is mirrored by a single SQLite table with
 * an `id TEXT PRIMARY KEY` + `payload TEXT NOT NULL` JSON-blob column +
 * `created_at TEXT NOT NULL`. We use the JSON blob shape (instead of
 * normalized columns) because:
 *   - The shape evolves rapidly (Sprint 16 D7 still iterating on the
 *     hash-chain payloads).
 *   - Per-table query patterns are simple (filter by `roundId`, `companyId`,
 *     `userId`) and the indexes we DO need can be added later as separate
 *     migrations once the shape stabilises.
 *   - We never touch these rows from a non-JS context, so the JSON blob
 *     is the canonical representation.
 *
 * Idempotency: `INSERT OR REPLACE INTO ... (id, payload, created_at)` on
 * every write so re-applying a mutation after a partial failure is safe.
 * ======================================================================== */

const COMMS_TIERS_TABLES = [
...
function ensureCommsTiersTables(): void {
  try {
    const db = rawDb();
    for (const tableName of COMMS_TIERS_TABLES) {
      db.exec(
    ...
    /* Lookup index for the per-round / per-user query patterns. SQLite
     * cannot index inside JSON without an expression, so we use json_extract
     * on demand and rely on the table cardinality (low to medium) being
     * fine without a covering index at launch volumes. */
  } catch (err) {
    log.warn("[commsTiersStore.ensureTables] failed:", (err as Error).message);
  }
}
ensureCommsTiersTables();

function persistRow(table: string, id: string, payload: unknown, createdAt: string): void {
  try {
    const db = rawDb();
    db.prepare(
    ...
    log.warn(`[commsTiersStore.persistRow ${table}] write failed:`, (err as Error).message);
  }
}
...
function loadRows<T>(table: string): T[] {
  try {
    const db = rawDb();
    const rows = db
      .prepare(`SELECT payload FROM ${table} WHERE deleted_at IS NULL ORDER BY created_at ASC`)
      .all() as Array<{ payload: string }>;
    const out: T[] = [];
    for (const r of rows) {
      try { out.push(JSON.parse(r.payload) as T); } catch { /* skip malformed */ }
    }
    return out;
  } catch {
    return [];
...
export function createCoInvestorGroup(args: { companyId: string; participants: string[]; actorId: string }): CoInvestorGroup {
  return withTrace(TRACE.formulaId, TRACE.version, TRACE.region, () => {
    ...
    persistRow("comms_co_investor_groups", g.id, g, g.createdAt);
...
export function postCoInvestorGroupMessage(args: { groupId: string; authorUserId: string; body: string }): CoInvestorGroupMessage {
  return withTrace(TRACE.formulaId, TRACE.version, TRACE.region, () => {
    ...
    persistRow("comms_co_investor_group_messages", m.id, m, m.createdAt);
...
export function requestCoInvestorIntro(args: { groupId: string; requesterId: string; targetId: string }): { id: string; status: string } {
  return withTrace(TRACE.formulaId, TRACE.version, TRACE.region, () => {
    ...
    persistRow("comms_intro_requests", r.id, r, r.createdAt);
...
export function setSoftCirclePeerOptIn(args: { roundId: string; userId: string; optedIn: boolean; crossCohortDmOptedIn?: boolean }): SoftCirclePeerOptIn {
  return withTrace(TRACE.formulaId, TRACE.version, TRACE.region, () => {
    ...
    /* Composite key (roundId+userId) used as the PK so re-opt-in upserts. */
    persistRow("comms_soft_circle_peer_opt_ins", `${args.roundId}:${args.userId}`, rec, rec.updatedAt);
...
export function setIoiPulse(args: { roundId: string; userId: string; pulse: IoiPulse }): IoiPulseRecord {
  return withTrace(TRACE.formulaId, TRACE.version, TRACE.region, () => {
    ...
    persistRow("comms_ioi_pulses", `${args.roundId}:${args.userId}`, rec, rec.updatedAt);
...
export function createEndorsement(args: { roundId: string; companyId: string; endorserUserId: string; chip: EndorsementChip; text: string; disclaimerAck: boolean }): Endorsement | { error: string } {
  return withTrace(TRACE.formulaId, TRACE.version, TRACE.region, () => {
    ...
    endorsements.push(e);
    persistRow("comms_endorsements", e.id, e, e.createdAt);
    ...
    persistRow("comms_high_value_advocates", args.endorserUserId, { userId: args.endorserUserId }, e.createdAt);
...
export function removeEndorsement(args: { id: string; founderUserId: string }): { ok: boolean; reason?: string } {
  return withTrace(TRACE.formulaId, TRACE.version, TRACE.region, () => {
    ...
    /* Re-persist with removedAt set so it survives restart. */
    persistRow("comms_endorsements", e.id, e, e.createdAt);
...
export function startCrossCohortDm(args: { roundId: string; fromUserId: string; toUserId: string; body: string }): CrossCohortDm | { error: string; status?: CrossCohortDmStatus } {
  return withTrace(TRACE.formulaId, TRACE.version, TRACE.region, () => {
    ...
    persistRow("comms_cross_cohort_dms", dm.id, dm, dm.createdAt);
...
export function muteCrossCohort(args: { roundId: string; muterId: string; mutedId: string }) {
  muteList.push({ ...args });
  const muteId = `${args.roundId}:${args.muterId}:${args.mutedId}`;
  persistRow("comms_mute_list", muteId, args, new Date().toISOString());
...
export function postQaQuestion(args: { roundId: string; askerUserId: string; body: string }): QaQuestion {
  return withTrace(TRACE.formulaId, TRACE.version, TRACE.region, () => {
    ...
    qaQuestions.push(q);
    persistRow("comms_qa_questions", q.id, q, q.createdAt);
...
export function postQaAnswer(args: { questionId: string; authorUserId: string; body: string }): QaAnswer | { error: string } {
  return withTrace(TRACE.formulaId, TRACE.version, TRACE.region, () => {
    ...
    qaAnswers.push(a);
    persistRow("comms_qa_answers", a.id, a, a.createdAt);
...
export function createDiligenceVolunteer(args: { roundId: string; volunteerUserId: string; softCirclerUserId: string }): DiligenceVolunteer {
  return withTrace(TRACE.formulaId, TRACE.version, TRACE.region, () => {
    ...
    persistRow("comms_diligence_volunteers", v.id, v, v.createdAt);
 ...
 * v25.4 — hydration
 *
 * Eager-load every Sprint 16 collection from its mirror table on module
 * boot, so a server restart never wipes Tier 1/2/3 comms state. Idempotent:
 * if the array is already populated (e.g. a test re-imported the module)
 * the hydration is skipped.
 * ======================================================================== */
export function hydrateCommsTiersStore(): void {
  try {
    if (coInvestorGroups.length === 0) {
      coInvestorGroups.push(...loadRows<CoInvestorGroup>("comms_co_investor_groups"));
    }
    if (coInvestorGroupMessages.length === 0) {
      coInvestorGroupMessages.push(...loadRows<CoInvestorGroupMessage>("comms_co_investor_group_messages"));
    }
    if (introRequests.length === 0) {
      introRequests.push(
        ...loadRows<{ id: string; groupId: string; requesterId: string; targetId: string; status: string; createdAt: string }>("comms_intro_requests"),
      );
    }
    if (softCirclePeerOptIns.length === 0) {
      softCirclePeerOptIns.push(...loadRows<SoftCirclePeerOptIn>("comms_soft_circle_peer_opt_ins"));
    }
    if (ioiPulses.length === 0) {
      ioiPulses.push(...loadRows<IoiPulseRecord>("comms_ioi_pulses"));
    }
    if (endorsements.length === 0) {
      endorsements.push(...loadRows<Endorsement>("comms_endorsements"));
    }
    if (crossCohortDms.length === 0) {
      crossCohortDms.push(...loadRows<CrossCohortDm>("comms_cross_cohort_dms"));
    }
    if (qaQuestions.length === 0) {
      qaQuestions.push(...loadRows<QaQuestion>("comms_qa_questions"));
    }
    if (qaAnswers.length === 0) {
      qaAnswers.push(...loadRows<QaAnswer>("comms_qa_answers"));
    }
    if (diligenceVolunteers.length === 0) {
      diligenceVolunteers.push(...loadRows<DiligenceVolunteer>("comms_diligence_volunteers"));
    }
    if (muteList.length === 0) {
      muteList.push(...loadRows<{ roundId: string; muterId: string; mutedId: string }>("comms_mute_list"));
    }
    if (highValueAdvocates.size === 0) {
      for (const row of loadRows<{ userId: string }>("comms_high_value_advocates")) {
        if (row?.userId) highValueAdvocates.add(row.userId);
      }
    }
    log.info(
      `[hydrate] commsTiersStore restored groups=${coInvestorGroups.length} messages=${coInvestorGroupMessages.length} ioi=${ioiPulses.length} endorsements=${endorsements.length} qa_q=${qaQuestions.length} qa_a=${qaAnswers.length} diligence=${diligenceVolunteers.length} advocates=${highValueAdvocates.size}`,
    );
  } catch (err) {
    log.warn("[hydrate] commsTiersStore read failed:", (err as Error).message);
  }
}

/* Eager-hydrate on module load — mirrors the pattern in consortiumLinkStore
 * and bulkMessageStore so DB state is in memory before the first request. */
hydrateCommsTiersStore();
