...
// v25.1 Bug 1 fix — commit-funded-batch derives shares from round.pricePerShare
// when the funded-queue entry shares="0" default (the wire-fund step doesn't ask
...
/* ===========================================================================
 * v25.4 — KYC enforcement on wire-funded transitions.
 *
 * The investor_kyc table (created by track1Routes.handleInvestorKyc) records
 * a row when an investor completes the in-platform KYC attestation. Until
 * this commit, the wire-funded endpoint only checked compliance holds; it
 * never verified KYC. For a regulated capital-markets platform that is a
 * real product gap — money must not move until KYC + accreditation are on
 * file for the investor.
 *
 * This helper returns the investor's KYC status; the wire-funded route
 * calls it before enqueuing the funded entry.
 *
 * Bypasses (configurable via env, all default OFF):
 *   - KYC_ENFORCE_WIRE_FUNDED=0   — disable the gate entirely (e.g. during
 *                                    a one-off backfill). Defaults ON.
 *   - The admin role always bypasses (admins resolve compliance manually).
 *
 * Failure modes returned by the gate:
 *   - { ok: false, code: "KYC_REQUIRED" }      no row in investor_kyc
 *   - { ok: false, code: "NOT_ACCREDITED" }    row exists but accredited=0
 * ======================================================================== */

export interface KycGateResult {
  ok: boolean;
  code?: "KYC_REQUIRED" | "NOT_ACCREDITED" | "DB_ERROR";
  message?: string;
  /** Resolution hint for the UI — deep-link to KYC for the investor. */
  resolutionUrl?: string;
}

export function checkInvestorKycForWireFunded(investorId: string): KycGateResult {
  if (process.env.KYC_ENFORCE_WIRE_FUNDED === "0") {
    return { ok: true };
  }
  if (!investorId) {
    return { ok: false, code: "KYC_REQUIRED", message: "Investor identity missing." };
  }
  try {
    const db = rawDb();
    /* Sort by created_at DESC to honour the latest attestation if the
     * investor re-submitted (e.g. accreditation status changed). */
    const row = db
      .prepare(
        `SELECT id, accredited, jurisdiction, created_at
           FROM investor_kyc
          WHERE investor_id = ?
      ...
      .get(investorId) as
      | { id: string; accredited: number; jurisdiction: string; created_at: string }
      | undefined;
    if (!row) {
      return {
        ok: false,
        code: "KYC_REQUIRED",
        message: "Investor must complete KYC before wire-funded transitions.",
        resolutionUrl: `/investor/kyc?investorId=${encodeURIComponent(investorId)}`,
      };
    }
    if (!row.accredited) {
      return {
        ok: false,
        code: "NOT_ACCREDITED",
        message: "Investor KYC on file but accreditation is not verified.",
        resolutionUrl: `/investor/kyc?investorId=${encodeURIComponent(investorId)}`,
      };
    }
    return { ok: true };
  } catch (err) {
    /* DB error — never silently allow money movement. Return a hard 500. */
    log.warn(
      "[captableCommitStore.checkInvestorKycForWireFunded] DB read failed:",
      (err as Error).message,
    );
    return { ok: false, code: "DB_ERROR", message: "KYC verification failed." };
  }
}
    ...
    try {
      ...
      db.transaction((tx: any) => {
        ...
        for (let e of candidates) {
          ...
          // v25.1 Bug 1 fix (Avi prod report 11-Jun):
          // The wire-funded enqueue step defaults shares="0" when the founder
          // doesn't pass an explicit shares value (the common case — the wizard
          // doesn't ask for shares per investor; it asks for $ wired). Then this
          // batch handler used to call isValidShares("0") → false → returns
          // "invalid_shares" with no fallback. The handler MUST derive shares
          // from round.pricePerShare × amount when the queue entry is "0".
          //
          ...
          if (!isValidShares(effectiveShares)) {
            try {
              ...
              if (pps && pps > 0) {
                ...
                // NOTE: BigInt literals (e.g. `1_000_000n`) require ES2020 target; this
                // codebase's tsconfig has no explicit target so we use BigInt("...") form
                ...
                if (ppsMicro > BigInt(0)) {
                  ...
                  if (derived > BigInt(0)) {
                    effectiveShares = derived.toString();
                    log.info(
                      `[commit-funded-batch] derived shares=${effectiveShares} for invitation=${e.invitationId} ` +
                      `(amount=${e.amount} ÷ pricePerShare=${pps})`
                    );
                  }
                }
              }
